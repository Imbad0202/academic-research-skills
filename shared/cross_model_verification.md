...
Kind ↔ result-shape pairs are closed: `design_freeze` and `editorial_decision` are `enum_comparison` (decision enums per the checkpoint table above); `da_critique` is `full_return`. Any other combination is malformed.
...
3. **Transport.** Execute § API Call Patterns with the **payload only** — `owner_decision` and everything outside the fences never reach the cross-model (blindness).
