...
Ren et al. (2026, arXiv:2607.13104, §8.1.2) state the same requirement from the judge side: judge-based measurement needs reliability evidence — repeated runs with variance estimates, aggregation across judge instances, and **calibration against a verifiable subset** via metric-based checks or targeted human review — plus transparency about the judge's identity, rubric, and budget, precisely because a system optimized against an uncalibrated judge over-optimizes to the judge's latent biases. Calibration mode is that safeguard instantiated: the user's gold set is the verifiable subset.
