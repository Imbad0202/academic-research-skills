...
- **Disclosure Mode**(academic-paper v2.9) — venue별 AI 사용 명시문 생성기. v1은 ICLR, NeurIPS, Nature, Science, ACL, EMNLP 커버. (이후 확장: v2 데이터베이스(#596)는 9개 의학 출판 정책 대상 — ICMJE, NEJM, The Lancet, JAMA, BMJ, PLOS, Frontiers, 그리고 데이터베이스 최초의 중국어 정책 대상 2건(출판사 전체에 적용되는 中华护理杂志社 항목과 단일 저널 国际眼科杂志) — 을 추가.)
