...
- When the joined evidence (`source_acquired`, `source_verified_against_original`, derived `human_read_source`, and — #513 — the governing mark's `read_scope` attestation) is unchanged between passes, the marker's resolved form is byte-identical to the prior pass.
