...
**Reset-boundary interaction (v3.6.3+):** the §"Passport Reset Boundary" emission sequence above invokes this same OR before computing the JCS hash. Otherwise `ARS_PASSPORT_RESET=1` on a `systematic-review` run would freeze the passport without `slr_lineage`, and the consuming `resume_from_passport=<hash>` session would see an empty `state_tracker.stages` + a flag-less incoming passport → OR resolves `false` → PRISMA-trAIce dispatch blocks. Computing `slr_lineage` before the reset-boundary hash ensures the field is hashed into the boundary (the hash is computed over the ledger after the new entry has its content, see `passport_as_reset_boundary.md` §"The reset boundary protocol" step 2).
