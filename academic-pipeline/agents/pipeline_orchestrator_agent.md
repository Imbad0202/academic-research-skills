...
**Idempotency:** the finalizer pass is idempotent on the join of `(literature_corpus[]` row, read-log row`)` for each slug — re-running on an already-resolved marker with the same input evidence yields byte-identical output. Concretely:

- A marker resolved to `<!--ref:slug ok-->` is **terminal-stable**: once a slug reaches all-true triple, no later pass demotes it (the user cannot un-acquire / un-verify / un-read a source mid-session).
- A marker resolved to `<!--ref:slug LOW-WARN-->` or to one of the `[UNVERIFIED CITATION ...]` visible warnings is **conditionally stable**: it stays as-is **iff** the input evidence is unchanged. When the user remediates (acquires the source, runs `/ars-mark-read <refcode>`, or rewrites the upstream entry's `source_verified_against_original`), the next finalizer pass re-evaluates the marker against the matrix and may promote it (e.g. `LOW-WARN` → `ok`). A marker is never demoted when evidence improves.

In other words: `ok` is a fixed point; lower statuses are upper bounds that improve as evidence accumulates. The finalizer always re-applies the matrix to every `<!--ref:slug ...-->` it can read.

**Revision loops:** on revision loops (Stage 4 → reviewer → Stage 4 revise; or `academic-paper` Phase 6 → Phase 4 loops), the finalizer re-runs against the current draft and resolves any newly-emitted bare `<!--ref:slug-->` comments introduced in the revision pass alongside re-evaluating the existing markers per the idempotency rule above. Resolved markers **do not invalidate** in the sense that the matrix never demotes them; they may, however, be promoted upward when remediation has occurred between passes.
