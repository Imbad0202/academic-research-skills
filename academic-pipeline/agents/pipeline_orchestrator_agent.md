...
| Stage 2 -> 2.5 | Complete Paper Draft + #547 scope context for Phase E4 (RQ Brief `scope` — the required E4 input; `sub_question_bindings` + outline section→sub-question map when present) + Schema 2 `search_strategy` (#548 — the E5 comparison basis, when a bibliography exists) | Schema 4 (Paper Draft) + Schema 1 scope fields + Schema 2 `search_strategy` | Pass to integrity_verification_agent |
...
| Stage 4/4' -> 4.5 | Revised/Re-Revised Draft + the same #547 scope context and #548 `search_strategy` as Stage 2 -> 2.5 | Schema 4 (revised) | Pass to integrity_verification_agent (final verification) |
