...
- **Audit artifact ledger (v3.6.7):** Schema 9's `audit_artifact[]` ledger ([`shared/handoff_schemas.md`](../../shared/handoff_schemas.md) "Audit Artifact Ledger") is also append-only and survives reset by the same mechanism as `reset_boundary[]` and `compliance_history[]` — the passport carries it intact across the session break. On `resume_from_passport`, the orchestrator does NOT replay prior audit runs; it re-verifies each persisted entry by reading the referenced `artifact_paths.{jsonl,sidecar,verdict}` files and re-running the eleven Layer 2 + Layer 3 gating checks at [`docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md`](../../docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md) §5.2. An entry whose underlying artifact files are missing, mutated (sha mismatch), or whose `deliverable_sha` no longer matches the current deliverable is rejected as stale and the corresponding gate must run a fresh audit before the resumed pipeline can transition. This closes the post-reset attack surface where a forged passport carries `verified_at` / `verified_by` timestamps but no recoverable evidence behind them.
