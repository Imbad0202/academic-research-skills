...
cache-through by default. Each resolver sends the same payload class — citation
metadata of your references (titles, DOIs/IDs, authors, years) — and all four work
...
|---|---|---|---|---|---|
| Chinese-literature resolver (`scripts/chinese_literature_client.py`, #595) | **Standalone CLI only — deliberately NOT part of the four-index verification gate** (the same reproducibility choice) | DOI prefixes / DOIs of the works you resolve | `doi.org` (RA lookup + resolution), `hdl.handle.net`, NCBI E-utilities (`eutils.ncbi.nlm.nih.gov`) | NCBI API key (optional; does not relax the client's polite pacing) | Don't invoke the CLI |
