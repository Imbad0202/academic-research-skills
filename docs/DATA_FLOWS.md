...
| SessionStart update check (`scripts/ars_update_check.sh`, #544; plugin installs only) | At most one curl transfer per 24 h (3-second total ceiling, redirects followed), silent on failure | **No user data** — fetches a public `plugin.json` and compares versions | `raw.githubusercontent.com` by default; `ARS_UPDATE_CHECK_REMOTE_URL` overrides the endpoint | none | `ARS_UPDATE_CHECK=0` |
