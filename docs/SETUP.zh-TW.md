...
| `ARS_CACHE_REVALIDATE=1` | Unreleased (#541) | 選擇性即時重驗：同時支撐 HIGH-IMPACT 主張（#549 分層）且超過時效門檻的參考文獻，改為繞過 cache 即時查驗（並回寫 cache）。預設關閉＝僅 advisory。 | `academic-pipeline/agents/integrity_verification_agent.md` § A0.5 |
