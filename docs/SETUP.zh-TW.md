...
| `ARS_MODEL_TIERING` | Unreleased (#517) | Opt-in 模型分層：`economy`（frontier session——execution 型 agent 降一階，樓地板 Opus 級）或 `quality-boost`（低於 frontier 的 session——judgment 型 agent 在 Stage 2.5/4.5 關卡與最終審查面跳升到 frontier 級）。未設定 = 全部用 session model；未知值警告一次後視同未設定。 | `shared/model_tiering.md` |
