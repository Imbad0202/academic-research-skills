...
**Scoped Phase 2B′ re-verification (who re-applies a criterion):** an answer of `original_upheld` (dissent) or `primary_revised` (divergence) MANDATES re-applying a criterion to the manuscript — a verification judgment the orchestrator (a dispatching layer, #523) must never make and a mechanical append cannot make. For each such answer the orchestrator dispatches a SCOPED fenced Phase 2B′ call restricted to the affected item(s): inputs are the item's Phase-1 record (the criterion to re-apply), the 2A input set (both manuscripts + diff/apply report), and the item's committed 2A verdict record; the Response Letter and all other items are WITHHELD. The call emits a `ReapplicationRecord` (§5.3) `{item_id, answer_ref (the adjudication/resolution id), reapplied_verdict, evidence_anchor, rationale}` — REQUIRED for every `original_upheld` / `primary_revised` answer even when the verdict does not change, so silent non-reapplication cannot pass; when `reapplied_verdict` differs from the row's `final_verdict`, the corresponding `AdjustmentRecord` (basis `cross_model_adjudication`, `source_ref` to the answer) is derived from it and the row updates. When cross-model is ACTIVE, the §9 judge's blind application already is this step — its output is recorded as the `ReapplicationRecord` (no extra call). 2B′ lint failure aborts with `phase2b_lint_failed`. Every surfaced outcome, deferred or final, is checker-verified, and no user-answerable state can bypass verification, die in an abort before the user was asked, or resolve without its mandated re-application.
...
| `academic-pipeline/references/pipeline_state_machine.md` (Material Transfer table only) | Parallel table gains the same Stage 3' → 4.5 sidecar row (cargo documentation on an EXISTING legal transition — no transition change, §15 holds) |
