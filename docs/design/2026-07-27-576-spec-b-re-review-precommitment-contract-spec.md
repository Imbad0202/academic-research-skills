...
`CrossModelResolution`: `{item_id, state: "primary_upheld" | "primary_revised", rationale}` — required for every P1 `diverges` row BEFORE a non-deferred `decision_state` may be emitted; while any is missing, the only checker-valid `decision_state` is `user_review_required` (§6 G2 — absence is the defer trigger, not a schema violation). No record exists (or is needed) for `agree` / `unavailable` / `not_configured` rows (§9).
...
- **Normalization grammar (emitter-side, mechanical), in declared order:** (1) STRIP every parenthetical group `(...)` and every ` — <suffix>` tail — specialty decorations are removed BEFORE splitting, so separators inside a parenthetical (e.g. `Peer Reviewer 3 (Cross-disciplinary/Practical)`) cannot fragment a token; (2) split on the separator set `{",", "/", ";", " and ", "&"}`; (3) trim whitespace; case-fold. A stripped token normalizes to a canonical label by: `EIC` / `Editor-in-Chief` → `EIC`; `R<n>` / `Reviewer <n>` / `Peer Reviewer <n>` → `R<n>`; `DA` / `Devil's Advocate` → `DA`. Unrecognized tokens are DROPPED (never guessed). The result, in original order and deduplicated, is `source_reviewer_labels`; the checker validates it against the closed vocabulary. PR-B1's checker fixtures include the existing example cards VERBATIM (`examples/hei_paper_review_example.md`, `examples/interdisciplinary_review_example.md` — both use the parenthetical Role form) as must-parse cases.
...
4. Recompute invariants: EVERY roadmap item, all priorities, has exactly one matrix row; every P1/P2 item has exactly one pre-commitment record and one verdict record; every P3 verdict record carries `applied_criterion: not_precommitted` (and only P3 may); `final_verdict != phase2a_verdict ⟺ adjustment` with closed-set basis and referential integrity (`from_verdict` = `phase2a_verdict`, `to_verdict` = `final_verdict`, unique resolvable ids); the G2 deferral biconditional — at least one pending user-input state (§6 G2 a/b/c) ⟺ `decision_state: user_review_required`, so a non-deferred decision requires zero pending states and a deferred one requires at least one (§7/§9/§6.4); escalation floors applied iff `approval_state: approved`; attribution evidence rules (§8); Step 1→2→3 of §6 recomputed from `DecisionInputs` AND independently from the raw records — both must equal the emitted `decision_state` (+ `abort_reason` when aborted). Mismatch → `[RE-REVIEW-ABORT: synthesis_mismatch]`, exit 1 (voids the synthesis, parity with #510).
