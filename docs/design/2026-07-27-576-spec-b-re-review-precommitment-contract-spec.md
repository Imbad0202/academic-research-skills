...
| `shared/handoff_schemas.md` | Schema 11 sidecar note (points at `traceability.schema.json`) + Schema 11 `status` enum gains `CANNOT_VERIFY` + the Validation rule "empty `authors_claim` → `verified: CANNOT_VERIFY`" is scoped to legacy mode (contract mode derives `verified` from `final_verdict`, §5.3) + Schema 6 `judge_record` gains the `precommitment_hash` field and the routing-degraded / not-run value extensions mirroring the protocol's Judge Record block |
