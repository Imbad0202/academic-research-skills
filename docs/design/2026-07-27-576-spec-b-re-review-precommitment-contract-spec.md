...
**Deferral loop (the ONLY place user input enters the derivation):** a pending user-answerable state never aborts and never races the checker — it DEFERS. The checker validates the deferral itself (while any G2 state is pending, `user_review_required` is the only checker-valid `decision_state`); the matrix and the pending items surface at the Stage 3' checkpoint; each loop iteration is ATOMIC and ORDERED: (1) the user answer (an `EscalationApproval`, a user-adjudicated `DissentAdjudication`, a `CrossModelResolution`) is recorded; (2) any 2B′ re-verification that answer mandates (below) is dispatched and COMPLETES, its `ReapplicationRecord` and any chain-appended adjustment/row updates incorporated; (3) only then is the §5.3 sidecar RE-PERSISTED (a fresh emission carrying `revision: n+1` + `supersedes_hash` of the prior emission; `verdict_record_hash` NEVER changes — the 2A artifact is immutable); (4) the §13 checker RE-RUNS on the complete emission; (5) the recomputed `decision_state` re-surfaces. The checker never sees an answer without its mandated reapplication. The loop repeats until no pending state remains.
