...
| G2 | Any PENDING user-input state: (a) a §7 dissent bound tripped while ANY dissent record — not only the triggering one — lacks its own `DissentAdjudication` (once a bound trips, EVERY dissent needs one; that is the covering cardinality); (b) a P1 `diverges` row with no `CrossModelResolution` — reachable ONLY when its mechanical re-application concluded `CANNOT_VERIFY` or could not run (the system path resolves every other diverges row BEFORE the first emission), so what the user decides is real: accept `primary_upheld`, or direct a guided re-examination; (c) an `EscalationExceptionRecord` whose `effective_approval_state` is `pending` (no `EscalationApproval` joined via `exception_id` yet) | `decision_state: user_review_required` — matrix + pending items delivered, decision deferred |
