...
`CrossModelResolution`: `{resolution_id, item_id, state: "primary_upheld" | "primary_revised", resolved_by: "judge" | "user", rationale}` — required for every P1 `diverges` row BEFORE a non-deferred `decision_state` may be emitted; while any is missing, the only checker-valid `decision_state` is `user_review_required` (§6 G2 — absence is the defer trigger, not a schema violation). No record exists (or is needed) for `agree` / `unavailable` / `not_configured` rows (§9).
...
| `academic-pipeline/SKILL.md` | Mirror surface of the state-machine boundary semantics (#528 keeps the two in lockstep) — the Stage 3' contract dispatch + sidecar forwarding notes mirror there, same-commit with any lock re-pin it needs |
