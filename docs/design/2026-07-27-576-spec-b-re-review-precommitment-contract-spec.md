...
`ResolutionIntent`: `{intent_id, item_id, intended_state: "primary_upheld" | "primary_revised" | null, answered_by: "user" | "system", conflicted: bool (default false)}` — the divergence answer's typed carrier (§6 loop); `intended_state: null` only with `answered_by: system` (the active-cross-model no-prejudgment form).

`CrossModelResolution`: `{resolution_id, item_id, intent_id, reapplication_id, state: "primary_upheld" | "primary_revised", resolved_by (copied from the intent's `answered_by`), rationale}` — required for every P1 `diverges` row BEFORE a non-deferred `decision_state` may be emitted; while any is missing, the only checker-valid `decision_state` is `user_review_required` (§6 G2 — absence is the defer trigger, not a schema violation). No record exists (or is needed) for `agree` / `unavailable` / `not_configured` rows (§9).
...
`DecisionInputs`: the mechanically-derived operands §6 Steps 2-3 consume — a compact per-P1-item list `{item_id, final_verdict, driving_severity: "critical" | "major" | "minor" | null, residual_magnitude?}` (`null` for `source_kind` items and legacy roadmaps whose Schema 7 `severity` is legitimately absent — B1 treats `null` as NOT critical, never fabricating a severity, and a `null` on a non-legacy finding-driven item fails the checker; B1's `MADE_WORSE × critical` join and B2-B4's quantifiers all recompute from this list); counts per final-verdict class per priority; per-priority `residual_magnitude` counts for `PARTIALLY_ADDRESSED` rows; `p2_addressed_rate {numerator, denominator}` (§6 definition); regression list with severities; frozen non-regression new-issue ids; the escalation summary `[{exception_id, effective_approval_state: "pending" | "approved" | "rejected", escalation_class, mechanical_decision_impact}]` — `effective_approval_state` is JOINED from the `EscalationApproval` via `exception_id` (`pending` when none exists; the `EscalationExceptionRecord`'s own field is the literal emission constant and is never copied here); the `reject_recommended` third disjunct's operands ride here, not just the floors; `reject_recommended: bool`; the `apply_chain_witness` closed composite (§11) — emitted so the checker recomputes Steps 2-3 from the same numbers the synthesizer used. Step 1 (G0-G2) is NOT recomputable from `DecisionInputs` and is not meant to be: the checker recomputes it from the raw artifacts + manifest directly.
...
**Deferral loop (the ONLY place user input enters the derivation):** a pending user-answerable state never aborts and never races the checker — it DEFERS. The checker validates the deferral itself (while any G2 state is pending, `user_review_required` is the only checker-valid `decision_state`); the matrix and the pending items surface at the Stage 3' checkpoint; each loop iteration is ATOMIC and ORDERED: (1) the answer is recorded — an `EscalationApproval` and a user-adjudicated `DissentAdjudication` are recorded DIRECTLY; a divergence answer is recorded as a typed `ResolutionIntent` (§5.3 — not yet a `CrossModelResolution`, whose `state` must derive from the reapplication and so cannot precede it); (2) the mandated 2B′ re-verification is dispatched against that intent and COMPLETES, its `ReapplicationRecord` (`answer_refs` naming `intent:<intent_id>`) incorporated; the `CrossModelResolution` is created — and any verdict-changing adjustment appended — ONLY when the derived state does not contradict a non-null `intended_state`; a contradiction creates NEITHER record: the intent is marked `conflicted: true`, the reapplication stands as evidence, the row's verdict is untouched, and the item stays pending under G2(b) (no resolution exists) with both positions shown to the user — never an abort, never a silent preference; (3) only then is the §5.3 sidecar RE-PERSISTED (a fresh emission carrying `revision: n+1` + `supersedes_hash` of the prior emission; `verdict_record_hash` NEVER changes — the 2A artifact is immutable); (4) the §13 checker RE-RUNS on the complete emission; (5) the recomputed `decision_state` re-surfaces. The checker never sees an answer without its mandated reapplication. The loop repeats until no pending state remains.

**Scoped Phase 2B′ re-verification (who re-applies a criterion):** an answer of `original_upheld` (dissent) or EITHER divergence resolution state (`primary_revised` AND `primary_upheld` — an "upheld" verdict claims a re-examination happened, so the re-examination must be witnessed by the same record; only `replacement_approved` needs none, its criterion having already been applied at 2A) MANDATES re-applying a criterion to the manuscript — a verification judgment the orchestrator (a dispatching layer, #523) must never make and a mechanical append cannot make. For each such answer the orchestrator dispatches a SCOPED fenced Phase 2B′ call restricted to the affected item(s): inputs are the item's Phase-1 record (the criterion to re-apply), the 2A input set (both manuscripts + diff/apply report), the item's committed 2A verdict record, AND a PROJECTION of the item's existing adjustment chain — `{adjustment_id, basis, from_verdict, to_verdict, evidence_anchor[] (with `anchor_artifact` tags), residual_gap?}` per record, `rationale` EXCLUDED (it may restate author persuasion) — booked anchors are admissible evidence per §3.4 regardless of tag (an `author_pointer_located_evidence` or letter-tagged `valid_rebuttal` anchor was already adjudicated at booking; withholding it would let a blind re-application reverse a legitimate upgrade merely by failing to re-find or re-derive it); the Response Letter itself and all other items are WITHHELD. The call emits a `ReapplicationRecord` (§5.3) `{reapplication_id, item_id, answer_refs: [tagged refs, 1+, closed grammar "adjudication:<dissent_id>" | "intent:<intent_id>"], pre_reapplication_verdict (the row's verdict at dispatch — the chain tail before this loop iteration, or `phase2a_verdict` when no chain; the TIME ANCHOR every comparison below binds to), reapplied_verdict, evidence_anchor[] | cannot_verify_reason (same discriminated union as a §3.3 verdict, each anchor carrying the same `anchor_artifact` tag as §5.3 — a re-application may legitimately conclude CANNOT_VERIFY), residual_gap? {text, residual_magnitude} (REQUIRED iff `reapplied_verdict = PARTIALLY_ADDRESSED`), rationale}` — every `original_upheld` adjudication and every `ResolutionIntent` MUST appear in exactly one record's `answer_refs`, even when the verdict does not change, so silent non-reapplication cannot pass (the `CrossModelResolution` is created AFTER and FROM the reapplication — §6 loop step 2 — never referenced by it); when a dissented item is ALSO cross-model-divergent, one blind application legitimately covers both answers (coalesced into one record's `answer_refs` — no duplicate call, no duplicate record). When `reapplied_verdict` differs from `pre_reapplication_verdict`, a new `AdjustmentRecord` is appended to the chain (basis `cross_model_adjudication`, `from_verdict` = `pre_reapplication_verdict`, `to_verdict` = `reapplied_verdict`, `source_ref` = `"reapplication:<reapplication_id>"`, `supersedes_adjustment_id` = the prior latest when one exists) and the row's `final_verdict` + `adjustment_id` update — the comparison is anchored to the RECORDED pre-value, never to the row after update. The judge-output shortcut applies ONLY to the §9.3 dissent path (the judge's blind application of the ORIGINAL criterion is recorded as that adjudication's `ReapplicationRecord` — no extra call). Divergence rows NEVER take the shortcut: the reapplication is always a fresh scoped 2B′ call by the routed seat verifier — the judge's own divergent pass is the thing being examined and cannot double as the witness (else `primary_upheld` would be structurally unreachable and the judge's vote would become the verdict). When cross-model is ACTIVE, the orchestrator emits the divergence `ResolutionIntent` itself with `answered_by: "system"` and `intended_state: null` (no prejudgment — a null intent never conflicts, the resolution derives purely from the reapplication); when NOT active, the user's intent carries their expected state and the conflict rule above applies. 2B′ lint failure aborts with `phase2b_lint_failed`. Every surfaced outcome, deferred or final, is checker-verified, and no user-answerable state can bypass verification, die in an abort before the user was asked, or resolve without its mandated re-application.
...
| B4 | Any P1 OR P2 `PARTIALLY_ADDRESSED` with `residual_magnitude: must_fix` (a must_fix-grade residual is decision-worthy regardless of the item's original priority — without this a P2 item could count toward the addressed rate while carrying blocking residual work) | **Major Revision** |
...
| B6 | Residual (provably: all P1 `FULLY_ADDRESSED` incl. `addressed_by_rebuttal`; `p2_addressed_rate ≥ 80%`; no P2 `MADE_WORSE`; no P2 `PARTIALLY_ADDRESSED` with a `must_fix` residual; no regression-attributed new issues) | **Accept** |
...
Every issue found during Phase 2A that is not traceable to a roadmap item gets a `NewIssueRecord`: `{new_issue_id, description, location_anchor, severity (Schema 6 vocabulary), attribution, attribution_evidence, nearest_roadmap_item: item_id | null, non_match_rationale}`. The last two are the TYPED non-match witness: the record names the closest roadmap item (or `null` when nothing is even adjacent) and states why the issue falls outside that item's criterion scope — so "not traceable" is an auditable claim, not an unrecorded judgment, and the checker can flag a `regression` whose `nearest_roadmap_item` names an item covering the same anchor span for review.
...
Schema: `input_manifest.schema.json` — `{round_id, artifacts: {original_manuscript, revised_manuscript, revision_roadmap, editorial_decision_letter, response_to_reviewers, revision_patches[], apply_reports[], round1_findings, round1_config_cards}, each a `present`-DISCRIMINATED union: `{present: true, path_or_passport_ref, sha256, version_label: string|null, origin_date: string|null}` or `{present: false}` (an absent artifact carries NO ref, hash, or freshness fields — no sentinel strings, no fabricated hashes; the §11 presence policies decide what each absence means). `path_or_passport_ref` is itself a TAGGED union with closed grammar `"passport:<artifact_ref>"` | `"path:<relative path>"` — the prefix IS the passport-tracked discriminator (checker-enforced; no untagged strings). The two freshness fields are copied from the artifact's Material Passport entry when the ref is `passport:`-tagged, else `null` (a `null` on a `passport:` entry fails the checker; `null` on a `path:` entry is a visible degradation, freshness rules skip it).
...
- **Apply-chain witness (the consumer-side witness, §1.1)** — one ORDERED-CHAIN rule set, hashes in the 12-hex `base_draft_hash` format from `ars_apply_revision_patch.py` (multi-round sequences are legal, reports ordered as listed in `apply_reports[]`, each paired with its `revision_patches[]` entry by position): the FIRST report's `base_draft_hash` must equal the manifest's `original_manuscript` hash prefix; each subsequent report's `base_draft_hash` must equal its predecessor's `output_draft_hash`; the LAST report's `output_draft_hash` — and only the last's — must equal the revised-manuscript hash prefix. Any broken link means some patch ran against a different text than the chain claims (a rewritten-after-apply draft breaks the last link; a wrong base skews every diff-supported `regression`/`previously_missed`/`MADE_WORSE` judgment) → `manifest_hash_mismatch`, G0 — the SD-9 channel this witness closes. The witness result is the CLOSED composite `apply_chain_witness ∈ {pass, fail, first_link_not_run (original_manuscript absent, inner+last links still checked), not_run_no_reports (apply_reports empty — legitimate outside-toolchain revision, never a silent vacuous pass)}` with declared PRECEDENCE `not_run_no_reports > first_link_not_run` (no reports means no links of any kind exist to check — the co-occurrence of both trigger conditions resolves to it), carried in `DecisionInputs` AND on a dedicated Judge Record `Apply-report chain` line (§16; it REPLACES the old Evidence-seen `output_draft_hash: match/MISMATCH` phrase). The patch/diff files themselves are the `revision_patches[]` manifest entries (hash-bound and tagged like every other artifact; a diff consumed at 2A but absent from the manifest violates the §11 same-input guarantee and fails the checker).
- Required-set policy: `revised_manuscript`, `revision_roadmap` are hard-required (absent → G0). `original_manuscript` absent → FOUR degradations, all visibly marked: (i) attribution — every new issue is `indeterminate` (§8), never a guess; (ii) `MADE_WORSE` — the discriminators (per-item and the §5.1 generic one) compare against the original text, so the verdict is assignable only where the diff/apply report supplies the pre-revision span; elsewhere it is unevaluable and the verdict follows the remaining criteria with a `[MADE-WORSE-UNEVALUABLE: no original manuscript]` marker; (iii) §6.4 escalation — the original-text anchor cannot be produced, so an exception cannot be substantiated (the `new_standard` stays advisory) with an `[ESCALATION-UNSUBSTANTIATABLE: no original manuscript]` marker; (iv) `change_summary` — when the apply reports are ALSO absent, both of its §3.3 sources are gone: it is then written from the revised manuscript alone and carries `[CHANGE-BASIS-ABSENT: no original manuscript, no apply report]` (the required field stays total on the degraded path — schema does not hard-fail). `response_to_reviewers` absent → Phase 2B runs claim-matching-empty (matrix `authors_claim` = "—"); commitments whose evidence class is `acknowledgment_only` keep their author-declared `fulfillment_status` UNCHANGED (the closed Schema 11 enum gains no value) but cannot be independently confirmed, so each affected commitment carries the report-level advisory flag `[COMMITMENT-EVIDENCE-ABSENT: acknowledgment_only — no response letter]`; the existing `COMMITMENT_GAP` rule applies unchanged. `editorial_decision_letter` absent → the §4 inheritance chain runs WITHOUT its level-2 layer (`inherited_criterion.letter_text` legitimately absent) and the run carries `[CRITERIA-LAYER-ABSENT: no decision letter]` alongside the existing `"unknown (provenance block absent)"` Judge Record provenance rule. `round1_findings` absent → the level-3 driving-finding layer degrades to the Schema 7 TRANSPORTED fields alone (severity/anchor/confidence ride on the roadmap items themselves, #574 A2/A3, so partial coverage survives) with `[ROUND1-FINDINGS-ABSENT]`. `round1_config_cards` absent → `[YARDSTICK-REGENERATED]` path. Every one of the NINE manifest artifacts now has a presence policy; every degradation is a visible marker, not a silent downgrade.
...
| `scripts/check_pipeline_boundary_semantics.py` | CONTENT_LOCKS sha256 re-pin, same commit, for the THREE locked files this delivery touches: `pipeline_orchestrator_agent.md`, `pipeline_state_machine.md`, `academic-pipeline/SKILL.md` (the lock otherwise fails CI on any byte change) |
| `academic-pipeline/SKILL.md` | Pipeline-narrative consistency (NOT a #528 mandate — the #528 lockstep pins the Stage 5/6 boundary sections, which this spec does not touch): the Stage 3' contract dispatch + sidecar forwarding notes mirror here so the SKILL's stage description does not drift from the orchestrator's |
| `shared/handoff_schemas.md` | Schema 11 sidecar note (points at `traceability.schema.json`) + Schema 11 `status` enum gains `CANNOT_VERIFY` + the Validation rule "empty `authors_claim` → `verified: CANNOT_VERIFY`" is scoped to legacy mode (contract mode derives `verified` from `final_verdict`, §5.3) + Schema 6 `judge_record` gains three OPTIONAL fields: `precommitment_hash`, `routing_status` (closed: `card_mapped` / the two `[ROUTING-DEGRADED: …]` values — `reviewer_configuration` is untouched), and `apply_chain_witness` (the §11 closed composite) |
