...
- `apply_reports[].output_draft_hash` must equal the manifest's revised-manuscript hash prefix (the 12-hex `base_draft_hash` format from `ars_apply_revision_patch.py`) — the consumer-side witness (§1.1). Mismatch = the draft was rewritten after apply → `manifest_hash_mismatch`, G0 abort (the untouched-block evidence is stale). SYMMETRICALLY on the pre-revision side, as an ORDERED CHAIN (multi-round sequences are legal): the FIRST report's `base_draft_hash` must equal the manifest's `original_manuscript` hash prefix, each subsequent report's `base_draft_hash` must equal its predecessor's `output_draft_hash` (reports ordered as listed in `apply_reports[]`, each paired with its `revision_patches[]` entry by position), and the LAST report's `output_draft_hash` must equal the revised-manuscript hash prefix — any broken link means some patch ran against a different text than the chain claims, so every diff-supported `regression`/`previously_missed`/`MADE_WORSE` judgment would run against a wrong baseline (`manifest_hash_mismatch`, G0 — the SD-9 channel this witness closes); `original_manuscript` absent → the first-link half is NOT-RUN, visibly (the same third `not run` value on the Judge Record's Evidence-seen line as below, plus the `DecisionInputs` witness status). The patch/diff files themselves are the `revision_patches[]` manifest entries (hash-bound and tagged like every other artifact; a diff consumed at 2A but absent from the manifest violates the §11 same-input guarantee and fails the checker). When `apply_reports` is EMPTY (a revision made outside the patch toolchain — legitimate per the protocol's input list), both witness halves are recorded as NOT-RUN with a visible `[APPLY-REPORT-ABSENT]` marker in the Judge Record and `DecisionInputs` — never a silent vacuous pass.
...
| `shared/handoff_schemas.md` | Schema 11 sidecar note (points at `traceability.schema.json`) + Schema 11 `status` enum gains `CANNOT_VERIFY` + the Validation rule "empty `authors_claim` → `verified: CANNOT_VERIFY`" is scoped to legacy mode (contract mode derives `verified` from `final_verdict`, §5.3) + Schema 6 `judge_record` gains the `precommitment_hash` field, a THIRD VALUE on the EXISTING `reviewer_configuration` field (`[ROUTING-DEGRADED: …]` — no new field), and the `not run` third value on the Evidence-seen `output_draft_hash` result |
