...
`PrecommitmentRecord`: `{item_id (Schema 7 id, e.g. REV-001), priority, inherited_criterion {roadmap_text, letter_text?}, operationalization {fully_addressed, partially_addressed, made_worse_discriminator}, expected_change_surface, equivalence_policy: "allowed", source_reviewer}`. `source_reviewer` is a VERBATIM copy of the Schema 7 item's required `reviewer` field (`handoff_schemas.md` RoadmapItem) — no new Schema 7 fields exist or are added; §10 defines the deterministic reviewer-label → seat routing over the Round-1 Configuration Cards. (Schema 7's `type` field is the revision-MAGNITUDE label `Major|Minor|Editorial` and is NOT a competence key — it plays no routing role.) Priority 2 lighter form: `operationalization.fully_addressed` only. Priority 3 (`consider`) items get NO pre-commitment record (§5.2 `not_precommitted`).
...
- `previously_missed` — present in the original manuscript; Round 1 missed it. Evidence: anchored in BOTH versions. Reported, CANNOT escalate the decision (only §6.4 overrides). Routed forward: when `decision_state` is Major Revision, it enters the new Stage 3' → 4' Roadmap as a `priority: consider` item whose `description` carries the `[PREVIOUSLY-MISSED: NEW-<n>]` prefix (Schema 7's closed priority enum is untouched — `consider` already has no decision effect, matching the non-escalation rule; the prefix preserves provenance); when `decision_state` is Accept or Minor Revision, the frozen `NewIssueRecord`s reach the Stage 4.5 final integrity check through an EXISTING channel: the traceability sidecar is Material Passport cargo (§5.3), and the Stage 4/4' → 4.5 handoff row plus `integrity_verification_agent`'s input list are extended so the gate receives and surfaces them (scheduled in §16 — no new artifact, no new transition). `decision_state: user_review_required` defers routing along with the decision. Both destinations exist in the current state machine — no new transition is created (the `Stage 4' → 3'` prohibition, `pipeline_state_machine.md` § Prohibited Transitions, is untouched).
...
- Each pre-commitment record carries `source_reviewer` — a verbatim copy of the Schema 7 item's REQUIRED `reviewer` field (§5.1); no Schema 7 change is needed and none is scheduled.
- Phase 2A dispatch routes on reviewer labels: the Round-1 Reviewer Configuration Cards define which seat each Round-1 reviewer label (EIC / R1 / R2 / R3 / DA) occupied — that card-declared mapping IS the routing table. An item routes to the seat of the FIRST label in its `source_reviewer` list that maps to a non-DA scoring seat; if no label does (DA-only items, `source_kind` items with editorial provenance, unmappable labels), it routes to EIC. Seat personas come from the frozen cards; the `verified_by` field records the seat. The DA seat is never a verification persona (its Round-1 role is adversarial challenge, not fix verification) — the map contains no DA target.
...
| `academic-pipeline/agents/pipeline_orchestrator_agent.md` | Stage 3' dispatch: manifest emission + three-call orchestration + abort surfacing; Stage 4/4' → 4.5 handoff row forwards the sidecar's frozen `previously_missed`/`indeterminate` records (§8) |
