...
>
> **Release semver:** **`v3.6.9`** (settled 2026-05-07 brainstorm Q7). The release tag `v3.6.8` was already consumed by the generator-evaluator contract (suite v3.6.8 SHIPPED 2026-05-03, main `734dd23`, see CHANGELOG `[3.6.8] - 2026-05-03`). This spec will ship as **v3.6.9** (patch line continuation) per the roadmap-slot-vs-release-semver decoupling pattern (see precedent: `2026-04-22-ars-v3.7.3-reading-check-probe-design.md` filename retains roadmap slot v3.7.3 while actual release tag is v3.5.1).
>
...
> | Q2 | `human_read_source: true` set via **explicit user command** (e.g. `/ars-mark-read <refcode>`); no inference, no session-activity heuristic | §3.1 frontmatter, §3.3 LOW WARN row, **NEW** §3.6 Human-Read Signal Protocol | Adds new subsection; closes §9 Q4 |
...
> | Q6 | D5 retrieval fallback chain = **default ON**, no env flag. Backward compat via early-exit on first content-match success (Risk 4 mitigation already specified). | §3.5 + §8 Risk 4 (add explicit "no opt-out flag" rationale) | None |
> | Q7 | Release tag = `v3.6.9`. Roadmap slot stays `v3.6.8`. | This banner + filename unchanged | Cross-doc sweep: any reference to "v3.6.8" as ship tag → "v3.6.9 (roadmap slot v3.6.8)" |
> | Q8 | D3 Cite-Time Provenance Check = **hybrid**: (a) downstream agents (`synthesis_agent`, `draft_writer_agent`) emit placeholder `[CITE-PROVENANCE-PENDING:refcode]` at every `[refcode]` emission **without reading frontmatter** (agents stay in-prompt-context); (b) `pipeline_orchestrator_agent` at stage transition reads frontmatter, applies §3.3 4-cell matrix, replaces placeholders with `[UNVERIFIED CITATION — NO ORIGINAL]` / `[UNVERIFIED CITATION — AI HAS NOT CROSS-CHECKED]` / removes (OK case) / appends per-section pre-finalization checklist (LOW WARN); (c) `format_converter_agent` hard-gates on any unresolved `[CITE-PROVENANCE-PENDING:` or `[UNVERIFIED CITATION` marker. **Architecturally mirrors v3.6.7 Step 6 audit_artifact gate** (agent emits artifact → orchestrator validates at stage gate). | §3.3 control flow rewrite + §4 Step 3 (split into 3a agent placeholder spec / 3b orchestrator finalize spec / 3c format_converter hard-gate spec) | Q4 ("every emission") superseded — placeholder is per-emission, finalization is per-stage. UX trade-off: user does not see resolved markers in-session until stage gate fires; mitigated by stage gate firing at every checkpoint, not only at end-of-pipeline |
>
...
**Status:** Design — spec input from real-world chapter-writing session, brainstorm-record-amended 2026-05-07
**Date:** 2026-04-30 (substance) / 2026-05-07 (brainstorm decisions)
**Target release:** **v3.6.9** (settled 2026-05-07 Q7; roadmap slot v3.6.8)
