...
**Reset-boundary path (codex round-2 [P2] closure):** under `ARS_PASSPORT_RESET=1` + `systematic-review` mode, the v3.6.3 reset protocol freezes a passport at the FULL checkpoint **and halts before the Stage 1 → Stage 2 handoff**. Handoff-only emission would therefore miss the only write opportunity, and the resuming fresh session would see an empty `state_tracker.stages` + flag-less incoming passport → OR resolves false → PRISMA-trAIce blocks. Fix: the orchestrator prose at §"Run-level lineage emission" specifies that `emit()` runs **before any passport write**, including the reset-boundary write (so the field is hashed into the boundary entry per the JCS protocol).
