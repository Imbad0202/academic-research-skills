...
- ⚠️ **`marketplace.json` schema differs from this roadmap's draft.** Correct shape:
  ```json
  {
    "name": "...",
    "plugins": [
  ...
  ```
  The roadmap's Task 1.3 used `plugins[].path` directly — that is wrong. Use `source: {source: "directory", path: "..."}` instead.
...
**重要事實（claude-code-guide 已查證）：**
- 第三方 plugin（如 ARS）預設 **auto-update OFF**
- 使用者要在 `/plugin` UI 切 marketplace 為 auto-update on
- 沒有 `/plugin update` 指令；Claude Code 啟動時檢查
- 手動 refresh：`/plugin marketplace update academic-research-skills`
...
> 沒開 auto-update 的話：手動跑 `/plugin marketplace update academic-research-skills`。
```
