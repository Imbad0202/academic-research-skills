...
- **Override is first-class — but it only re-screens if Phase 1 re-runs.** The scholar may change the profile by re-running intake (a fresh `academic-paper` invocation, or an in-session correction that re-runs the intake step). Intake overwrites the PCR `Domain Evidence Profile` row with the new effective value. **The load-bearing caveat:** the profile is consumed *only* by `literature_strategist_agent` in Phase 1. An override recorded **before Phase 1 runs** is fully effective (the consumer reads the updated row). An override recorded once the corpus is already fixed — i.e. **Phase 1 has already completed** (e.g. while drafting at Stage 4; the revision loop `8→5→6`, `academic-paper/SKILL.md:270`, does not re-run Phase 1) **OR Phase 1 was skipped entirely** (a complete bibliography was handed in, so literature search never ran) — updates the PCR row but does **not** retroactively re-screen the corpus. To avoid a *placebo* override (scholar changes the profile expecting a different corpus but the corpus is frozen), `intake_agent` MUST, when it records a profile override and Phase 1 has either already run **or been skipped** in this pipeline, surface a one-line `[PROFILE-OVERRIDE-NO-RESCREEN]` advisory: "the literature corpus is already fixed (screened under the previous profile, or handed in pre-built); to apply this profile, re-run literature search (Phase 1)." The override is honored for any *future* Phase-1 run, and the scholar is told plainly that the current corpus is not retroactively changed. (No silent no-op; the scholar decides whether to re-run Phase 1.)
...
- **Phase-1-will-be-skipped carve-out (no placebo prompt):** the profile's *only* consumer is Phase 1 (`literature_strategist`). When the run will **skip Phase 1 entirely** because a complete bibliography was handed in (`deep-research → academic-paper` handoff with a finished bibliography: `deep-research/SKILL.md:354` "Has Bibliography → skip literature search"; `academic-paper/SKILL.md:139` "User can skip Phase 1 if providing own sources"), Step 12 MUST NOT prompt the scholar to select a profile that nothing will read. Instead, intake records `unknown_user_defined` in the PCR row and surfaces a one-line `[NO-PROFILE-NEUTRAL]` advisory explaining *why*: "this run uses the handed-in bibliography and skips literature search, so a domain evidence profile would have no effect; to apply a profile, re-run literature search (Phase 1)." This prevents a mandatory placebo interaction. (When Phase 1 *will* run — the common handoff case where only redundant questions are skipped but literature search still runs — Step 12 prompts normally; the profile has a live consumer.)
...
| `deep-research → academic-paper` handoff, **Phase 1 still runs** (bibliography not complete / literature search still needed) | **yes** — intake_agent runs and only *skips redundant questions* (`intake_agent.md:37-50`, `deep-research/SKILL.md:352`); it skips Step 1 topic, **not** Step 12 | **PCR row set by Step 12** (intake asks the profile; inferred discipline MAY pre-fill the suggested default, scholar confirms) | **profile applies** — confirmed paper-intake path with a live Phase-1 consumer |
| `deep-research → academic-paper` handoff with a **complete bibliography → Phase 1 skipped** (`deep-research/SKILL.md:354`, `academic-paper/SKILL.md:139`) | intake runs, but Step 12 takes the **Phase-1-will-be-skipped carve-out** (Change B): no placebo prompt | PCR row recorded `unknown_user_defined` + `[NO-PROFILE-NEUTRAL]` advisory explaining the corpus is pre-built and a profile would have no consumer | **neutral, surfaced** — the scholar is told why a profile is not asked, and that re-running Phase 1 would let one apply. NOT a placebo. |
...
**Correction vs R1–R6 framing:** the `deep-research → academic-paper` handoff with a live Phase 1 is the *common* path and it **does** run `intake_agent` (skipping only redundant questions), so Step 12 runs and the profile IS asked. It is a profile-applies path, not a missing-profile path. The genuine **always-absent-row** paths are `plan → full` and true mid-entry (intake never sets the row). **Resume is row-based, not always-absent** — it keeps whatever the prior intake wrote, so it is a missing-profile path *only* when the prior intake left the row absent. And the bibliography-complete handoff sub-case is absent-by-carve-out (Step 12 records neutral + `[NO-PROFILE-NEUTRAL]` rather than prompting). Resolve by reading the row; the table classifies paths only to confirm each was considered.
