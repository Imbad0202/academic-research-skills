...
```
| **Domain Evidence Profile** | [profile_value / unknown_user_defined] |
```

`profile_value` is the resolved **effective** profile (one of the 4 ship-ready enum values). The PCR holds the *currently effective* profile — the active state, not a history. Override is "re-run intake, the row is overwritten" (same lifecycle as every other PCR row). There is **no append-only `selections[]` ledger** (R7 design decision: the PCR has no history mechanism — Style Profile does not have one either — and adding one would be unrequested abstraction for a single Phase-1 consumer; see "Override" below for what is and is not retained).
...
- **Override is first-class.** The scholar may change the profile by re-running intake (a fresh `academic-paper` invocation, or an in-session correction before Phase 1 that re-runs the intake step). Intake overwrites the PCR `Domain Evidence Profile` row with the new effective value. The scholar may know their sub-field better than any inferred default.
- **No intake ⇒ no profile row ⇒ neutral (the only fallback shape).** Any run that does not pass through `intake_agent` leaves the PCR `Domain Evidence Profile` row absent or set to `unknown_user_defined`: a standalone `deep-research` run, a mid-entry pipeline start (e.g. entering at Stage 2.5 with an existing draft), a `deep-research → academic-paper` handoff that arrives with a bibliography and **skips Phase 1 search**, or a `plan → full` transition that runs literature_strategist from plan artifacts without a full intake. In every such case `literature_strategist_agent`, if it runs at all, takes the neutral `unknown_user_defined` fallback. This is intentional and safe: with no scholar-confirmed profile, neutral evidence standards are the conservative default. The profile is "asked at intake when intake runs" — there is no claim that every entry path asks for it. See "Missing-profile gate" for the explicit per-path handling.
...
> Round history: R1–R6 carried the profile on the Material Passport as "Schema 13" with up to 6 edits (orchestrator producer + two consumers + in-place edit of the deep-research reference file). The R7 PCR-native rewrite collapses this to **3 edits** with no passport, no Schema number, and no edit to any deep-research file: **B (intake produces the PCR row), C (new academic-paper profile reference doc), E (literature_strategist consumes the PCR row).** There is no Change A (no Schema), no Change D (no second consumer).

### B. Intake production — `academic-paper/agents/intake_agent.md`

`intake_agent` is the **producer** of the PCR `Domain Evidence Profile` row. Add a profile-selection step as a **new intake step** (numbered after the current Step 11 Funding, e.g. **Step 12: Domain Evidence Profile**), and add the `| **Domain Evidence Profile** | ... |` row to the Paper Configuration Record output table (after the `Style Profile` row).
...
- **Mid-pipeline override:** if the scholar later changes the profile (a fresh `academic-paper` invocation that re-runs intake, or an in-session correction before Phase 1), `intake_agent` overwrites the PCR row. Because the only consumer runs once in Phase 1 after intake, the override is visible to it whenever it is recorded before Phase 1.
...
- **(a) row absent** (no intake / plan mode / mid-entry / handoff-skips-Phase-1) → neutral `unknown_user_defined`.
- **(b) row = `unknown_user_defined`** → neutral (the explicit default).
...
- **(d) discipline mismatch** (profile implies a discipline ≠ PCR `Discipline`) → proceed with both signals in their lanes, **emit `[PROFILE-DISCIPLINE-MISMATCH]`** (per the reconcile rule); this is a warning, not a fallback — admissibility still uses the profile.
...
def admit(source):
    if neutral_gate_admits(source):
        return True                          # never tighten: anything neutral admits stays admitted
    if profile != unknown_user_defined and profile_admits(source, profile):
        return True                          # only loosen: profile can ADD admit paths
    return False
# admit() is the union (OR) of neutral and profile admissibility. Under unknown_user_defined it == neutral_gate_admits exactly.
```

The sub-edits, all reverting to current behavior under fallback cases (a)/(b)/(c):
...
2. **Quality quick-assessment checklist** (`### Literature Quality Quick Assessment Checklist`, ~line 436): the existing 5-item quick-assessment total score penalizes preprints/archival. Under a non-neutral profile, a source passes if it **meets the neutral quick-assessment outcome OR meets the profile's evidence-type expectations** (union, not replacement — a source that passes the neutral score must still pass even if it does not match the profile).
3. **Quality gates** (`## Quality Gates`, ~line 468): the `>= 70% peer-reviewed` (line 479) and `>= 50% currency` (line 480) pass criteria contradict `humanities_interpretive` (primary/older canonical texts) and `cs_ml` (preprints). Make these gates profile-relative **in the loosening direction only**: under a non-neutral profile the admissible set *expands* (preprints count toward the `cs_ml` peer-reviewed-equivalent ratio; canonical texts do not count against `humanities_interpretive` currency). The thresholds are never raised, and a corpus that passes the neutral gate always passes the profile-relative gate.
...
The "no profile ⇒ neutral" rule must hold across **every** path that reaches `literature_strategist_agent`, not just a clean intake. Each path below lands on the neutral `unknown_user_defined` fallback (Change E case (a)), and that is correct behavior, not a defect:

| Path to literature_strategist | Profile state | Result |
|---|---|---|
| Standard `academic-paper full` with intake | PCR row set by Step 12 | profile applies |
| `plan → full` transition (literature_strategist runs from plan artifacts) | plan intake is exempt → no row | neutral fallback |
| `deep-research → academic-paper` handoff **with bibliography** that skips Phase 1 search (`deep-research/SKILL.md:352`) | literature_strategist may not run; if it runs, no intake row | neutral fallback (and if it does not run, the profile is simply not consulted) |
| Mid-entry pipeline start (e.g. Stage 2.5 with existing draft) | intake skipped → no row | neutral fallback |
| Standalone `deep-research` run | never reaches academic-paper intake or literature_strategist | profile not in play |

The gate is therefore not a separate code path — it is the single rule "resolve from the PCR row; absent ⇒ neutral" (Change E case (a)) applied uniformly. The table exists so a reviewer can confirm each run shape was considered; there is no run shape where a missing profile breaks the pipeline.
...
11. **No intake ⇒ no profile ⇒ neutral, across every path.** Any path that does not run a full intake — standalone `deep-research`, mid-entry start, `deep-research → academic-paper` handoff that skips Phase 1, `plan → full` — leaves no PCR profile row; `literature_strategist_agent`, if it runs, takes the neutral `unknown_user_defined` fallback (INVARIANT 4 case (a)). The Missing-profile gate table enumerates each path. The profile is "asked at intake when intake runs"; there is no claim that every entry path asks for it.
...
1. `intake_agent.md` documents the 4 effective enum values + the 5 reserved values; the PCR output table contains a `Domain Evidence Profile` row; the request/effective coherence rule and the reserved-fallback advisory text are present (INVARIANTS 1, 2, 12).
...
3. The consumer agent (`literature_strategist_agent`) contains a "resolve `domain_evidence_profile` from the PCR row" instruction AND the graceful-fallback cases (absent / `unknown_user_defined` / non-enum incl. hallucinated) AND the `[PROFILE-DISCIPLINE-MISMATCH]` reconcile advisory (INVARIANTS 4, 10). It also confirms `source_verification_agent` is NOT given a profile-resolution step.
...
7. The PCR `Domain Evidence Profile` row is named in `intake_agent.md` as the single carrier; no `Schema 13` / `selections[]` / Material-Passport reference is used for the profile anywhere (a guard against passport-carrier regression) (INVARIANT 3).

**Negative fixtures + mutation test (`scripts/test_check_domain_evidence_profile.py`):** deliberately (a) add a 5th effective enum value, (b) rename the gaps column back to "disqualifying", (c) strip one graceful-fallback case, (d) delete the preserved Medicine/Education text, (e) **remove the Policy fold reference**, (f) **add a `## Domain Evidence Profiles` section to `source_quality_hierarchy.md`** (must FAIL — the R-5 leak guard), (g) **introduce a `selections[]` / `Schema 13` / Material-Passport reference for the profile** (must FAIL — the carrier-regression guard) — each must make the checker FAIL (so it cannot trivially accept-all). Wired into `.github/workflows/spec-consistency.yml`.
