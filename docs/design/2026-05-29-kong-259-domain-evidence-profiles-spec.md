...
- **Override is first-class — but it only re-screens if Phase 1 re-runs.** The scholar may change the profile by re-running intake (a fresh `academic-paper` invocation, or an in-session correction that re-runs the intake step). Intake overwrites the PCR `Domain Evidence Profile` row with the new effective value. **The load-bearing caveat:** the profile is consumed *only* by `literature_strategist_agent` in Phase 1. An override recorded **before Phase 1 runs** is fully effective (the consumer reads the updated row). An override recorded **after Phase 1 has already completed** — e.g. while drafting at Stage 4 — updates the PCR row but does **not** retroactively re-screen the already-built corpus, because the revision loop (`8→5→6`, `academic-paper/SKILL.md:270`) does not re-run Phase 1. To avoid a *placebo* override (scholar changes the profile expecting a different corpus but the corpus is frozen), `intake_agent` MUST, when it records a profile override and Phase 1 has already run in this pipeline, surface a one-line `[PROFILE-OVERRIDE-NO-RESCREEN]` advisory: "the literature corpus was already screened under the previous profile; to re-screen, re-run literature search (Phase 1)." The override is honored for any *future* Phase-1 run (e.g. a fresh invocation), and the scholar is told plainly that the current corpus is not retroactively changed. (No silent no-op; the scholar decides whether to re-run Phase 1.)
...
- **(a) row absent** (no full intake: plan→full, true mid-entry, or resume — see Missing-profile gate; the `deep-research → academic-paper` handoff is NOT here, it runs intake) → neutral `unknown_user_defined`, **and in `full` mode emit a one-line `[NO-PROFILE-NEUTRAL]` advisory** so the neutral default is visible, not silent.
...
| `deep-research → academic-paper` handoff **with bibliography** | **yes** — intake_agent runs and only *skips redundant questions* (`intake_agent.md:37-50`, `deep-research/SKILL.md:352`); it skips Step 1 topic + literature search, **not** Step 12 | **PCR row set by Step 12** (intake still asks the profile; inferred discipline from the handoff MAY pre-fill the suggested default, scholar confirms) | **profile applies** — this is a confirmed paper-intake path, NOT a fallback path. Only if Phase 1 (`literature_strategist`) is itself fully skipped (bibliography already complete) is the profile simply not consulted. |
...
**Correction vs R1–R6 framing:** the `deep-research → academic-paper` handoff is the *common* path and it **does** run `intake_agent` (it only skips redundant steps), so Step 12 runs and the profile IS asked there. It is therefore a profile-applies path, not a missing-profile path. The genuine missing-profile paths are the *intake-skipping* ones (plan→full, true mid-entry, resume). This corrects the earlier table row that wrongly implied the handoff produces no profile row.
...
11. **No full intake ⇒ no profile ⇒ neutral, across every such path.** Any path that does not run a full intake — standalone `deep-research`, true mid-entry start, `plan → full` (simplified intake omits Step 12), resume-from-checkpoint — leaves no PCR profile row; `literature_strategist_agent`, if it runs, takes the neutral `unknown_user_defined` fallback (INVARIANT 4 case (a)) and emits `[NO-PROFILE-NEUTRAL]` in full mode so the neutral default is visible. **The `deep-research → academic-paper` handoff is NOT such a path** — it runs `intake_agent` (skipping only redundant questions, not Step 12), so the profile IS asked and the row IS set; it is a profile-applies path. The Missing-profile gate table enumerates every path and which side it falls on. The profile is "asked at intake when intake runs"; there is no claim that every entry path asks for it, but no path degrades quality invisibly.
...
3. The consumer agent (`literature_strategist_agent`) contains a "resolve `domain_evidence_profile` from the PCR row" instruction AND the graceful-fallback cases (absent / `unknown_user_defined` / non-enum incl. hallucinated) AND all four advisory tags it is specified to emit — `[NO-PROFILE-NEUTRAL]` (absent-row, full mode), `[PROFILE-UNRESOLVED]` (non-enum), `[PROFILE-DISCIPLINE-MISMATCH]` (reconcile) — plus the universal-gate carve-out language ("relevance / methodology / predatory still apply") so the monotonic contract is documented (INVARIANTS 4, 5, 10). It also confirms `source_verification_agent` is NOT given a profile-resolution step. (`[PROFILE-OVERRIDE-NO-RESCREEN]` is emitted by `intake_agent`, checked in item 1, not here.)
...
7. The PCR `Domain Evidence Profile` row is named in `intake_agent.md` as the single carrier (INVARIANT 3). **Carrier-regression guard (scoped to avoid false-fail):** within the **profile's own implementation instructions** — i.e. the Step 12 block in `intake_agent.md` and the profile-resolution block in `literature_strategist_agent.md` — the profile MUST NOT be stored on or resolved from a `Schema 13` number, a `selections[]` ledger, or the Material Passport. The guard checks only those instruction blocks (delimited by their section headings), **not** the whole repo and **not** historical/design-rationale prose: this spec deliberately mentions `Schema 13` / `selections[]` / Material Passport as the *superseded* R1–R6 design, and unrelated repo files legitimately use Material-Passport / Schema language for other features (e.g. `shared/handoff_schemas.md`, `draft_writer_agent.md`). Those are out of the guard's scope. (A whole-repo or whole-spec `anywhere` match would false-fail on this contrast language — the guard is intentionally narrowed to live profile instructions.)
