...
- Cache schema: `${ARS_CACHE_DIR}/claim_audit_v1/<cache_key_sha256>.json` filesystem KV. File body is a complete `claim_audit_result` entry verbatim (including the original `judge_run_at` from the judge invocation that produced it). Cache replay re-emits this entry unchanged so the result schema's `judge_run_at` required + `additionalProperties: false` rules remain satisfied. Cache-side metadata (mtime, "when did this row enter the cache") lives on the filesystem (`stat`), never inside the JSON body.
