...
(b) Judge response cache keyed by `(claim_text_hash, ref_slug, anchor_kind, anchor_value_hash, retrieved_excerpt_hash, judge_model)`. SHA-256 of the canonical-form joined string. **Cache lookup runs AFTER retrieval, not before** — including the `retrieved_excerpt_hash` in the key means that if the user later uploads a manual PDF, corrects the corpus entry, or the slug resolves to a different excerpt for the same anchor, the cache miss forces fresh judging rather than reusing a stale verdict tied to the old source text. Cache lives at `${ARS_CACHE_DIR}/claim_audit_v1/` (env var, default `~/.cache/ars/claim_audit_v1/`). Cache eviction is user responsibility (`rm -rf`); no automatic TTL — judgments are deterministic given the same six-tuple.

(c) Calibration: single gold-set covers both SUPPORTED/UNSUPPORTED/AMBIGUOUS/RETRIEVAL_FAILED judgments AND negative-constraint violation judgments. Gold tuple shape: `{claim_text, ref_text_excerpt, anchor, expected_judgment, optional_violated_constraint_id}`. FNR/FPR computed jointly across both judgment types.
...
- Cache schema: `{cache_key_sha256: {judgment, rationale, judge_model, cached_at}}` one JSON file per key (filesystem KV).
...
- New trigger boundary: claim_ref_alignment_audit_agent dispatches AFTER finalizer pass (Stage 5→Stage 6 transition) but BEFORE terminal gate. This mirrors v3.6.7 Step 6 (audit between deliverable completion and downstream consumption) without sharing infrastructure.
