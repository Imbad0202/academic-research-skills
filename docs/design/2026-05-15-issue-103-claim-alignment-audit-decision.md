...
(b) Judge response cache keyed by `(claim_text_hash, ref_slug, anchor_kind, anchor_value_hash, judge_model)`. SHA-256 of the canonical-form joined string. Cache hits skip judge invocation. Cache lives at `${ARS_CACHE_DIR}/claim_audit_v1/` (env var, default `~/.cache/ars/claim_audit_v1/`). Cache eviction is user responsibility (`rm -rf`); no automatic TTL — judgments are deterministic given the same inputs and judge model.
