...
Step 6 introduces one Schema 9 optional field plus three new standalone schemas under `shared/contracts/`. None of the existing schemas (Schema 9 base shape, Schema 13/13.1, `literature_corpus_entry.schema.json`, `reset_ledger_entry.schema.json`) is modified — every change is additive.
...
Required fields: `stage`, `agent`, `deliverable_path`, `deliverable_sha`, `run_id`, `artifact_paths`, `verdict`.
...
2. `prompt.bundle.primary_deliverables[].sha` matches the corresponding `audit_artifact_entry.deliverable_sha`.
3. `runner.git_sha` is a valid commit in the current repo (not a fabricated hash).
4. `timing.ended_at - timing.started_at == duration_seconds` within ±1 second tolerance.

These four cross-file checks are the Layer 3 contribution beyond Layer 2's per-row schema. A forger now needs to: produce N consistent JSONL rows (Layer 2), produce a sidecar whose `jsonl_session_id` matches all N rows (Layer 3 rule 1), pin the deliverable SHA correctly (Layer 3 rule 2), use a real repo commit (Layer 3 rule 3), and keep timestamps arithmetically self-consistent (Layer 3 rule 4). Cumulative forgery cost is high enough that running real codex is the cheaper path.
...
- `--bundle-id` is the same opaque string for all files in the bundle. Orchestrator reads this to know "these N audit_artifact entries belong to one logical bundle."

For Phase 2 chapter ships, three `audit_artifact[]` entries (one per primary deliverable) all carry the same `bundle-id`. Orchestrator's stage-transition gate (§5.1) accepts the bundle as transitioned only when all three entries' verdicts are PASS.
...
### 5.2 The eight verification checks

For each `audit_artifact[]` entry that gates a transition, orchestrator runs eight checks in order. The first failing check stops verification; orchestrator does NOT compose findings across checks. Reasoning: any check failure is sufficient cause to reject the artifact; running more checks just buries the diagnostic.

**Layer 2 checks (codex JSONL evidence):**

| # | Check | Implementation |
|---|---|---|
| L2-1 | JSONL file exists at path declared in `artifact_paths.jsonl` | Filesystem stat |
| L2-2 | Every JSONL row validates against `audit_jsonl.schema.json` | JSON Schema validator |
| L2-3 | All rows in JSONL share the same `session_id` | Set of distinct `session_id` values has size 1 |
| L2-4 | Last row has `role == "assistant"` and contains `final_message` carrying severity-bucket count summary | Regex match against final row's `final_message` |

**Layer 3 checks (sidecar evidence):**

| # | Check | Implementation |
|---|---|---|
| L3-1 | Sidecar file exists at `artifact_paths.sidecar` | Filesystem stat |
| L3-2 | Sidecar validates against `audit_sidecar.schema.json` | JSON Schema validator |
| L3-3 | Sidecar `session.jsonl_session_id` matches every JSONL row's `session_id` | String equality |
| L3-4 | Sidecar `runner.git_sha` resolves to a real commit in current repo | `git cat-file -e <sha>^{commit}` |

**Composite (Layer 2 + Layer 3 cross-evidence — implicit in the eight; not a ninth check):**

- L2-3 ensures internal JSONL consistency.
- L3-3 cross-bridges JSONL ↔ sidecar.
- Together, a forger needs to produce N JSONL rows with a stable session_id AND a sidecar that names the same session_id AND a real git_sha. The arithmetic and timing fields in sidecar (per §3.4 cross-file rules) raise the bar further.

**Verdict file is read AFTER all eight checks pass.** Orchestrator does NOT read `verdict.yaml` until Layer 2 + Layer 3 are clean; this prevents a forged verdict from leaking into orchestrator state via partial evidence.

### 5.3 Verdict semantics and ship/block decisions

Once eight checks pass, orchestrator parses `verdict.yaml` and decides per these rules:
...
**Override caveat:** `another_round` does NOT increment `target_rounds` automatically — wrapper next invocation still uses default 3. This is intentional: each escalation is a fresh user-owned decision, not a path that drifts toward unbounded iteration.
...
| ARS_PASSPORT_RESET reset boundary | Audit_artifact[] entries serialize into passport per Schema 9 append-only ledger semantics. On `resume_from_passport=<hash>`, orchestrator re-reads audit_artifact[] from passport and re-runs the eight verification checks — they MUST still pass even after session reset. (Resolves L3 from §2.3 — see §9 R4 for full risk discussion.) |
...
**Procedure.**
1. Read passport `audit_artifact[]` for an entry matching:
...
2. If no matching entry: BLOCK transition. Display
...
3. If matching entry exists: run the eight verification checks (§5.2 of
   Step 6 spec). First failure stops; surface the failed check.
4. If all eight pass: parse verdict.yaml. Apply ship/block per §5.3:
   ...
   - MATERIAL → BLOCK; surface findings; await re-audit
5. After 3 MATERIAL rounds: emit ESCALATION block per §5.4; await user
   ship_with_known_residue / another_round / abort_stage choice.
