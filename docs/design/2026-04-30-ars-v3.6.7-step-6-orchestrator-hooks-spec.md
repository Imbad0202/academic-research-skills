...
| `verdict.status` | string | enum: `["PASS", "MINOR", "MATERIAL", "AUDIT_FAILED"]`. Three completion states + AUDIT_FAILED for wrapper-side aborts (§4.6). AUDIT_FAILED proposals must pass schema validation in §5.6 Path B4 so Path B5 can short-circuit on them; rejecting AUDIT_FAILED at the schema layer would deadlock the failure-signaling channel. Cross-field rule (lint-enforced) below limits AUDIT_FAILED to zero finding counts plus required `failure_reason`. |
...
```json
{
  ...
  "oneOf": [
    ...
    {
      "title": "persisted",
      "description": "Orchestrator-merged, post-verification",
      "properties": {
        "verdict": {
          "type": "object",
...
- `verdict.status == "AUDIT_FAILED"` requires `p1 == 0 AND p2 == 0 AND p3 == 0` AND `failure_reason` is a non-empty one-line string. (See §3.5 + §4.6.)
...
sha_deliverable=$(sha256sum "$DELIVERABLE" | awk '{print $1}')
...
audit_template_path="shared/templates/codex_audit_multifile_template.md"
audit_template_sha=$(sha256sum "$audit_template_path" | awk '{print $1}')
declare -a primary_shas supporting_shas
for path in "${PRIMARY[@]}"; do
  primary_shas+=("$path:$(sha256sum "$path" | awk '{print $1}')")
done
for path in "${SUPPORTING[@]}"; do
  supporting_shas+=("$path:$(sha256sum "$path" | awk '{print $1}')")
done
...
bundle_manifest_sha=$(sha256sum "$OUT_DIR/$run_id.manifest.txt" | awk '{print $1}')
...
`verdict.verified_at` and `verdict.verified_by` are intentionally absent. Schema validation at step 4 enforces their absence in proposal files via `--mode proposal`; their presence is treated as schema error.
