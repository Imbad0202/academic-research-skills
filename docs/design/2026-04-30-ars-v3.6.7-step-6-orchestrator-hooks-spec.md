...
|---|---|---|
| 0 | Audit completed, all four contract files written (jsonl + sidecar + verdict + proposal entry) | Read verdict, proceed per §5.3 / §5.6 Path B |
...
| Step | Actor | Action |
|---|---|---|
| 1 | Wrapper | Produces three artifact files (jsonl / sidecar / verdict, §4.3) under `--output-dir`, atomically with tmp-file-rename |
...
| 3 | Orchestrator | At stage transition, follows the §5.6 Path A / Path B selection: Path A if a merged persisted entry already exists in passport for `(stage, agent, deliverable_sha)`; Path B if scanning `--output-dir` finds an unmerged proposal file |
| 4 | Orchestrator (Path B only) | Validates proposal against `audit_artifact_entry.schema.json --mode proposal` (rejects if `verdict.verified_at` / `verdict.verified_by` are pre-filled — Pattern C3 attack surface) |
| 5 | Orchestrator | Runs the nine gating checks (§5.2) using the paths declared in the entry's `artifact_paths` |
| 6 | Orchestrator | Validates `<run_id>.verdict.yaml` against `audit_verdict.schema.json`. On Path A also confirms verdict file's mirrored fields match the persisted entry's mirror (drift = forgery evidence) |
| 7 | Orchestrator (Path B only) | If all nine + verdict schema pass: fills `verdict.verified_at` (current UTC) and `verdict.verified_by = "pipeline_orchestrator_agent"`, re-validates against `--mode persisted`, appends to passport `audit_artifact[]` |
| 8 | Orchestrator (Path B only) | Moves the proposal file to `<output-dir>/consumed/<run_id>.audit_artifact_entry.json` so it is not re-merged on a later transition |
| 9 | Orchestrator | If any check fails: leaves the proposal file in place (Path B) or leaves the persisted entry untouched (Path A), surfaces the failed check to user (per §5.6 block message), no passport mutation |
...
**Procedure (two paths — one for already-merged entries, one for unmerged proposals).**
...
A2. Validate the entry against `audit_artifact_entry.schema.json --mode persisted`
    (`verdict.verified_at` and `verdict.verified_by` MUST be present).
A3. Re-run the nine gating verification checks (§5.2). On resume the L3-3 / L3-4
    SHA recompute is what catches deliverable mutation since the original session.
A4. If all nine pass: read `<run_id>.verdict.yaml`, validate against
    `audit_verdict.schema.json`. On schema failure, BLOCK with diagnostic. On
    schema pass, also confirm the verdict file's mirrored fields
    (`verdict_status`, `round`, `target_rounds`, `finding_counts`) match the
    persisted entry's mirror — drift is treated as Pattern C3 evidence
    (verdict file was forged or corrupted post-merge) and BLOCKS. If both
    checks pass, apply ship/block per §5.3. **Do NOT append or mutate the
    entry** — it is already in passport.
A5. If any check fails: BLOCK transition. The persisted entry is now stale (or
    forged); orchestrator does NOT delete it (append-only ledger semantics) but
    REQUIRES a fresh audit round. Surface block with re-audit command (Path B
    next round).

**Path B — proposal file (first-time merge after fresh audit run):**

B1. Path A1 found no matching merged entry. Scan `--output-dir` for an unmerged
    `<run_id>.audit_artifact_entry.json` matching the same `(stage, agent, deliverable_sha)`
    tuple. (Files in `consumed/` subdirectory are excluded — already merged.)
B2. If no proposal file either: BLOCK transition. Display
...
B3. Validate the proposal against `audit_artifact_entry.schema.json --mode proposal`.
...
B9. After round == target_rounds rounds with MATERIAL: emit ESCALATION block per
    §5.4; await user ship_with_known_residue / another_round (bumps cap by 1) /
    abort_stage choice.

**Path selection invariants:**
- Path A and Path B are mutually exclusive. Path A is taken whenever a matching
  merged entry exists; Path B is taken only when no merged entry exists.
- Path A never appends to passport (entry already there). Path B always appends
  exactly one persisted entry per successful merge.
- A merged-then-failed (Path A4 fail) does NOT trigger Path B in the same
  transition — orchestrator surfaces the block and waits for the user to dispatch
  a fresh wrapper run, which produces a new proposal that Path B will pick up
  on the next transition attempt.
