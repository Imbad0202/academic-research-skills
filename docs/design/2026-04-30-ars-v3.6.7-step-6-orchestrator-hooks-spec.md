...
This also corrects this spec's own §1.2 line 29 ("two-tier (18 micro + 1 integration)") and §2.1 Q4 row ("18 per-pattern micro-fixtures + 1 chapter-level integration fixture") which were drafted before §7's inventory walk surfaced the off-by-one. Both shipped lines should be read as "17 micro + 1 integration"; the shipped wording stays intact for now to preserve PR #50's squash-merge boundary, but L-doc-1 below carries this spec's §1.2 / §2.1 alongside the upstream main spec lines as part of the same docs-only follow-up.
...
```json
{
  ...
  "type": "object",
  "required": ["pattern_id", "agent", "stage", "fixture_kind", "upstream_context", "bad_run", "good_run"],
  "properties": {
    "pattern_id": {
      "type": "string",
      "enum": ["A1", "A2", "A3", "A4", "A5",
...
```yaml
run_id: fixture-A1-bad-2026-04-30T15-00-00Z-0a01
...
- §4.3 output construction: `<run_id>` generation per F1 regex, four contract files written to `<output-dir>/<run_id>.{jsonl,meta.json,verdict.yaml,audit_artifact_entry.json}` plus the two non-contract diagnostic files (`<run_id>.stdout`, `<run_id>.stderr`).
...
**Verification gate:** wrapper passes a self-test invocation (`scripts/run_codex_audit.sh --dry-run --stage 2 --agent synthesis_agent --deliverable <test-file> --round 1`) producing a well-formed proposal entry that validates against the Phase 6.2 schemas in `--mode proposal`. Manual smoke test against a known synthetic deliverable: all four contract files appear with correct shapes; AUDIT_FAILED path is exercised by injecting a SHA mutation. Full integration verification against fixtures defers to Phase 6.8.
