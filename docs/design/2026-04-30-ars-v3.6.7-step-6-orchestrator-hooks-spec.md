...
- Exactly one `item.completed` event with `item.type == "agent_message"` carries the verdict text. The wrapper extracts this via `parse_audit_verdict.py` (§4.4 Step 4); the verdict text format is contracted by `shared/templates/codex_audit_multifile_template.md` Section 6 (severity-bucket count summary), NOT by this schema. Note "exactly one" applies to the *final verdict* `agent_message`; intermediate `agent_message` `item.completed` events (codex thinking aloud between tool calls) are allowed in tool-using runs and the parser must select the *last* `agent_message` `item.completed` as the verdict-bearing event.
- `item.started` events appear only in tool-using runs and pair 1:1 with subsequent `item.completed` events sharing the same `item.id`. They are evidence that codex actually invoked tools (anti-fake-audit signal); verdict logic ignores them.
