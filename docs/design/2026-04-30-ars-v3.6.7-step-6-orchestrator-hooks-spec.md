...
| A7 | JSONL stream: every `item.started` event has a matching `item.completed` event sharing the same `item.id`; pairing is FIFO per `item.id`; no `item.completed` references an `item.id` that did not first appear in an `item.started` (with the documented exception that the *final* `agent_message` `item.completed` may appear without a prior `item.started` — codex emits the closing assistant message as a single completed event in no-tool runs and as the trailing event in tool-using runs). Stream-level invariant added to fill the L2-4 scope-defer (`audit_jsonl.schema.json` is per-row only; this is the only stream-shape rule that needs lint coverage in v3.6.7) | JSONL | both | `scripts/check_audit_artifact_consistency.py` (Phase 6.3) at orchestrator §5.2 read time | orchestrator BLOCKs with "audit fake-detected: tool event pairing violation: <details>" |
