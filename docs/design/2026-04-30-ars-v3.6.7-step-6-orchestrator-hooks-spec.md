...
4. **Audit artifact lifecycle observed.** §5.6 persists PASS / MINOR / MATERIAL proposals at B8d/B9 BEFORE applying ship/block at B10, so the lifecycle test splits by failure phase, not by ship/block outcome. For proposals that pass B6 / B7 / B8a and reach B10 — including BAD cases with MATERIAL verdicts — the harness verifies exactly one persisted `audit_artifact[]` entry is appended with `verified_at` populated by `_next_verified_at_ms` before ship/block is applied. For failures that stop before B8d/B9 — AUDIT_FAILED short-circuits at B5, ten-gate failures at B6, verdict-schema / verdict-mirror failures at B7, late-freshness failures at B8a, snapshot races at B8c, persisted-mode re-validation failures at B8d, passport-write failures at B9 — the harness verifies no persisted entry was appended (per the §5.6 verification failure state inventory's "Passport mutation = none" column).
