...
| L2-5 | JSONL stream tool-event 1:1 pairing per §3.7 A7: for every non-`agent_message` tool `item.id` the stream contains exactly one `item.started` and exactly one `item.completed` with that id (no duplicate starts, no duplicate completions, no orphan completions); parallel tool starts may complete in any order; `agent_message` `item.completed` events are exempt (codex emits assistant messages as single completion events) | Invoke `scripts/check_audit_artifact_consistency.py --check A7 --jsonl <path>`; the script implements §3.7 family A row A7 stream-shape rule | §3.7 A7 |
