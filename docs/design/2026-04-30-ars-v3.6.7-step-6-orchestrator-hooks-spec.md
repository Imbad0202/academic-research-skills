...
# Step 4 — produce verdict file. Two branches by codex exit code AND parse probe.
...
if [ "$exit_code" -eq 0 ] && scripts/parse_audit_verdict.py --probe "$OUT_DIR/$run_id.jsonl" >/dev/null 2>&1; then
  ...
  emit_audit_failed_verdict \
    ...
    > "$OUT_DIR/$run_id.verdict.yaml"
  # If we entered this branch with codex exit 0 (parse-probe failure path),
  # override wrapper_exit_code to 70 (EX_SOFTWARE) so the §4.6 contract holds:
  # an AUDIT_FAILED verdict file MUST be paired with non-zero wrapper exit.
  # If codex itself already exited non-zero, wrapper_exit_code already carries
  # that value and the override is a no-op.
...
| 70 (`EX_SOFTWARE`) | codex CLI exited non-zero (network failure, model error, partial JSONL) **OR** codex exited 0 but `parse_audit_verdict.py --probe` rejected the JSONL (missing `final_message`, malformed final row, etc.). The §4.4 Step 4 `wrapper_exit_code` override raises codex's exit-0 to 70 in the parse-probe-failure case so the wrapper's process exit always agrees with the verdict it just wrote (§4.6 contract: exit 0 ↔ PASS/MINOR/MATERIAL; non-zero ↔ AUDIT_FAILED) | jsonl (possibly partial), sidecar (with `process.exit_code` carrying codex's actual exit; may be 0 in the parse-probe-failure case — sidecar records the codex process's exit, wrapper exit is a separate signal), verdict (carrying status `AUDIT_FAILED`), proposal entry (carrying `verdict.status: AUDIT_FAILED`, `finding_counts: {p1: 0, p2: 0, p3: 0}`, `verdict.failure_reason: <one-line>`) | §5.6 Path B5 catches `AUDIT_FAILED` proposal: BLOCK transition without running the ten gating checks (audit did not complete; gating is meaningless) and without merging into passport |
