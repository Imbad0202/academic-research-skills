...
audit_artifact:                            # optional, added v3.6.7 Step 6
  - stage: 2                               # pipeline stage at which audit fired
    ...
    verdict:                               # parsed from verdict.yaml; mirrored here for fast reads
      status: MINOR                         # one of PASS | MINOR | MATERIAL | AUDIT_FAILED (this example shows MINOR because p3=1; PASS requires all-zero per §3.2 cross-field rule)
      ...
      finding_counts:
        ...
        p2: 0
        p3: 1
      verified_at: 2026-04-30T15-23-11Z    # when orchestrator verified Layer 2+3 passed (persisted entry only)
...
|---|---|---|
| `verdict.verified_at` | MUST be absent (`not.required`) | required; RFC 3339 datetime, UTC (`Z` suffix) |
...
```json
{
  ...
  "oneOf": [
    {
      "title": "proposal",
      "description": "Wrapper-emitted, pre-verification",
      "properties": {
        "verdict": {
          "type": "object",
          "not": {
...
- `verified_at` / `verified_by` are set ONLY at lifecycle step 8 (= §5.6 Path B8) by orchestrator. A wrapper-emitted proposal file with these fields filled is treated as malformed (Pattern C3 attack surface) and rejected at lifecycle step 4 schema validation (= §5.6 Path B4).
...
| **MATERIAL + acknowledgement** | latest entry's `verdict.status == "MATERIAL"` AND latest entry carries `acknowledgement` object whose `finding_ids` covers EVERY current `findings[].id` from the companion verdict file | Proceed to next stage; emit FULL checkpoint with `[Audit: MATERIAL at round N, residue acknowledged by user at <acknowledged_at>]` line. Partial coverage (acknowledgement listing some but not all current finding IDs) treated as MATERIAL for ship/block purposes — partial residue acknowledgement is NOT permitted; user must either acknowledge all findings or pick `another_round` / `abort_stage`. Lint enforces full-coverage at acknowledgement-write time per §5.4. |
