...
The exact wording matters. "External review" is intentionally generic — it does not name codex, does not name the orchestrator, does not say "my output will be audited later." Earlier drafts that wrote "the orchestrator runs codex audit afterward" leaked the trigger. The §6.3 sweep retires every variant of that leak.
...
| INV-2 | The PATTERN PROTECTION (v3.6.7) block contains zero sentences matching any Clause 2 violation pattern | Regex set against four pattern families: (a) `\bthe orchestrator\b.*\baudit\b`, (b) `\bcross-model audit (follows\|covers)\b.*codex_audit_multifile_template`, (c) `\baudit (afterwards?\|will be run\|is dispatched)\b`, (d) `\bdownstream audit\b\|\bthis output (is\|will be) audited\b` | `[INV-2] <file>:<line>: Clause 2 violation: <matched sentence>. Sentence must be removed per docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md §6.2.` |
| INV-3 | No file outside the three v3.6.7 downstream agent prompts gains a Clause 1 line as a side effect of the sweep (defends against well-intentioned over-application that would mark the L2 deferred question (§9 L2) as decided) | Manifest-driven file list in `scripts/v3_6_7_inversion_manifest.json` (contains exactly three filenames); lint scans repo for the canonical Clause 1 line and reports any hit outside the manifest | `[INV-3] <file>: canonical Clause 1 line found outside the v3.6.7 inversion manifest. If this is intentional widening, update scripts/v3_6_7_inversion_manifest.json AND open the L2 question per §9.` |
...
**Schema 13.1 contract for micro-fixture directories:**
...
```json
{
  ...
  "type": "object",
  "required": ["pattern_id", "agent", "stage", "fixture_kind", "bad_run", "good_run"],
  "properties": {
    "pattern_id": {
      "type": "string",
      "enum": ["A1", "A2", "A3", "A4", "A5",
...
**`expected_audit_findings.yaml` shape** (mirrors `audit_verdict.schema.json` finding shape from §3.5 so the same validator can verify both the fixture's expectation and the runtime verdict file):
...
| pattern_id | agent | stage | What BAD triggers | What GOOD avoids |
|---|---|---|---|---|
| A1 | synthesis_agent | 2 | Same source's legal effect drifts across two narrative sections | Pre-listed effect inventory + cross-section consistency check |
| A2 | synthesis_agent | 2 | Pending-verification entry treated as fact | Hedge wrap: "pending verification of X" |
| A3 | synthesis_agent | 2 | Citation mis-anchored to a paper that does not contain the claim | One-line anchor justification per substantive claim |
| A4 | synthesis_agent | 2 | Quoted phrase scope creep (verbatim quote extends beyond the verified phrase boundary) | Verbatim quote bounded; surrounding context paraphrased |
| A5 | synthesis_agent | 2 | Sibling-document fabrication (declarative claim about a chapter not in ground truth) | Conditional language ("if document X argues Y, ...") |
| B1 | research_architect_agent | 2 | Confidentiality / anonymity / pseudonymization terms used interchangeably in consent script | Each term passed through `irb_terminology_glossary.md` |
| B2 | research_architect_agent | 2 | Pseudo-reverse-coded item (different construct flagged as "reverse-coded") | One-line construct-equivalence justification |
| B3 | research_architect_agent | 2 | Calendar-anchored retrospective item without common event date | Event-anchored phrasing default |
| B4 | research_architect_agent | 2 | Leading-frame item phrasing imports chapter argument vocabulary | Neutral / balanced item phrasing |
| B5 | research_architect_agent | 2 | List-of-options item subsets / over-sets the primary-source list | Primary-source list declared and enumerated fully |
| C1 | report_compiler_agent | 5 | Compression overclaim (abstract drops protected hedge identified upstream) | Protected hedging phrases preserved per `protected_hedging_phrases.md` |
| C2 | report_compiler_agent | 5 | Reflexivity disclosure with deictic temporal phrase ("during this period") | Explicit year range / past-tense disambiguating verb / "former" prefix |
| C3 | report_compiler_agent | 5 | Output metadata claims audit-passed state (sub-agent fake-audit hallucination) | Canonical Clause 1 line obeyed (no audit-passed metadata) |
| D1 | cross_cutting | 2 | Multi-file deliverable audited file-by-file (sequential) misses cross-file inconsistency | Multi-file parallel audit per audit template §4.1 |
| D2 | cross_cutting | 2 | Single-round audit declares CLEAN (convergence theatre) | 3+ round iteration; CLEAN only after round-N zero-finding |
| D3 | cross_cutting | 2 | PARTIAL closure marked as CLOSED | PARTIAL stays PARTIAL until verdict file shows zero-finding |
| D4 | cross_cutting | 5 | Word count uses regex hyphenated-as-1, busts publisher hard cap | Whitespace-split + 3–5% buffer per `word_count_conventions.md` |

D-row fixtures have `agent: cross_cutting` because they are cross-cutting orchestration lessons rather than single-agent patterns. Their micro-fixture `stage` field reflects where the lesson is most likely to fire in a real pipeline; the harness exercises them by simulating the relevant stage's deliverable shape.
...
**Fixture artifacts** (Schema 13.1 integration-shape):
...
**Integration manifest** extends the micro-fixture schema:
...
This exercises every Path A / Path B / Path B-supersession code path in §5.6 within one fixture, without requiring real codex execution.

### 7.4 Success criteria

A pattern is considered "demonstrated triggered + protected" by the Step 8 fixture set when ALL of the following hold:

1. **BAD case triggers audit finding.** Running the harness on `<pattern_id>/bad_run/` produces an orchestrator decision matching `expected_orchestrator_action.yaml`. For 16 of 17 micro-fixtures this is `MATERIAL` with a finding in the expected dimension; for D2 the expected action is "round 1 PASS but D2 trigger logged" (D2 is about convergence-theatre detection, not P1 finding emission).
...
4. **Audit artifact lifecycle observed.** For successful merges, the harness verifies a persisted `audit_artifact[]` entry appears in the synthesized passport with `verified_at` populated by `_next_verified_at_ms`. For BAD-case BLOCKs, the harness verifies no persisted entry was appended.
...
Both jobs run on every PR touching `deep-research/agents/synthesis_agent.md`, `deep-research/agents/research_architect_agent.md`, `deep-research/agents/report_compiler_agent.md`, `academic-pipeline/agents/pipeline_orchestrator_agent.md`, `scripts/run_codex_audit.sh`, `scripts/check_audit_artifact_consistency.py`, `scripts/check_v3_6_7_pattern_protection.py`, `shared/contracts/audit/**`, `shared/contracts/passport/audit_artifact_entry.schema.json`, or `tests/fixtures/v3_6_7_pattern_eval/**`. This is the same path-filter discipline `.github/workflows/pytest.yml` already uses for v3.6.4 corpus adapters.
...
The §1–§8 design has accumulated four risks and four open questions that did not have enough surface area to settle within their introducing sections. They are collected here so a single review pass can read all of them together; each carries the load-bearing forward references from earlier sections so the trail is not lost.
...
2. `docs/PERFORMANCE.md` (added v3.4) gains a Step 6 onboarding subsection covering codex CLI installation, credential setup, and the wrapper's required environment variables (added in §10 Phase 6.7).
...
1. Every §3 / §4 / §5 section MUST read consistently with §1.2's threat-model statement. The §3.7 invariants index rows that touch Layer 2 / Layer 3 verification (B1–B7, E9) carry their own load-bearing pointers back to §1.2.
...
These are deferred from §2.3 L1–L3 plus one new question surfaced in §7.
...
**L-doc-1 — Upstream "18 patterns" prose retirement (new in §7.1).** The v3.6.7 main spec ([`2026-04-29-ars-v3.6.7-downstream-agent-pattern-protection-spec.md`](2026-04-29-ars-v3.6.7-downstream-agent-pattern-protection-spec.md)) §1, §2.1, §9 Step 8, and §10 each refer to "18 distinct patterns" but the inventory in §3 + §4 enumerates 17 numbered IDs (5+5+3+4). Step 6 / Step 8 fixtures track the 17-ID inventory (per §7.1). Forward reference: §7.1 → here. Status: **open; retirement is a docs-only follow-up.**

The retirement requires updating four prose locations in the main spec to "17 distinct patterns" without changing any of the 17 numbered IDs. The change is mechanical and ships under a separate docs-only PR after Step 6 lands; it is not a v3.6.7 spec amendment because the inventory itself is correct, only the summary count was off. The §10 Phase 6.8 implementation note logs this as a follow-up ticket so it does not silently absorb.
...
**Deliverable:** `scripts/run_codex_audit.sh` implementing §4 contract end-to-end.

**Includes:**
- Bash 4+ shebang and dependency check (`set -euo pipefail`).
- §4.2 input parsing (`--stage`, `--agent`, `--deliverable`, `--round`, `--target-rounds`, `--bundle-id`, `--bundle`, `--audit-template`, `--output-dir`, `--previous-findings`, optional `--dry-run`). EX_USAGE on missing required flags.
- §4.3 output construction: `<run_id>` generation per F1 regex, four artifact files written to `<output-dir>/<run_id>.{jsonl,meta.json,verdict.yaml,audit_artifact_entry.json}`.
...
- §4.4 helper functions: `_atomic_write`, `_sha256`, `_extract_jsonl_session_id`, `_compute_bundle_manifest_sha` per §3.6 contract, `_now_rfc3339_ms`, `_random_4hex` from `/dev/urandom`.
- §4.4 SIGTERM trap that aborts with EX_TEMPFAIL and cleans up partial artifact files (F-059 closure).
- §4.4 TOCTOU detection: re-stat the deliverable file just before writing the sidecar's `primary_deliverables[].sha`; abort with EX_USAGE if the file's mtime changed since `--deliverable` was first stat'd (F-060 closure).
- §4.6 failure-mode handling: emit AUDIT_FAILED proposal + sidecar + verdict file when codex CLI exits non-zero or when JSONL is unparseable; still write the proposal entry last so orchestrator's lifecycle invariant E8 holds.
- §4.5 multi-file Phase 2 audit support via `--bundle <manifest.json>`.
...
**Verification gate:** Phase 6.8 lint script reports zero violations against the wrapper output for all 17 micro-fixture BAD/GOOD pairs. Manual smoke test: run wrapper against a known synthetic deliverable, verify all four artifact files appear with correct shapes.
...
**Verification gate:** lint passes on the four schemas of Phase 6.2 + the orchestrator prompt of Phase 6.6 + the wrapper of Phase 6.1's output for all 17 micro-fixtures. Test suite achieves 100% coverage over §3.7 family rows.

**Dependencies:** Phase 6.2 (imports schemas).
...
- `scripts/check_pattern_eval_manifest.py` validating each fixture's `manifest.json` against the §7.2 / §7.3 schema.
- `scripts/test_pattern_eval_runtime.py` test harness running the orchestrator gate procedure against each fixture's BAD/GOOD pair.
- `.github/workflows/spec-consistency.yml` extended with `pattern-eval-unit` + `pattern-eval-integration` jobs per §7.6.
- L-doc-1 follow-up ticket logged in `docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md` §9.2 (this spec) — the upstream "18 patterns" → "17 patterns" prose retirement is tracked as an issue or TODO file note, not a v3.6.7 spec amendment.
...
**Dependencies:** Phase 6.1 (wrapper) + Phase 6.2 (schemas) + Phase 6.3 (lint) + Phase 6.6 (orchestrator prompt) + Phase 6.7 (downstream prompts). Phase 6.8 is the integration phase that exercises everything earlier phases produced.
