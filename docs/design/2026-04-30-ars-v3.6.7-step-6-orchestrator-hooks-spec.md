...
- `item.started` events appear only in tool-using runs and pair 1:1 with subsequent `item.completed` events sharing the same `item.id`. They are evidence that codex actually invoked tools (anti-fake-audit signal); verdict logic ignores them. **Pairing is a stream-level invariant, not a per-row schema rule** — Layer 2 (`audit_jsonl.schema.json`) validates each row's shape and §5.2 L2-3/L2-4 cover the canonical opening / closing / final-`agent_message` slots; the 1:1 `item.started` ↔ `item.completed` pairing check (`every item.started has a matching item.completed; no dangling pairs; ordering is FIFO per item.id`) lives in `scripts/check_audit_artifact_consistency.py` (Phase 6.3) as a stream-level rule, alongside the §3.7 family rules. Tool-event pairing was added to the canonical wire-format description here so implementers know the shape, but the gating burden stays per-row at Layer 2 by design (Q3 chose minimal Layer 2 surface; stream-level is Phase 6.3 territory).
