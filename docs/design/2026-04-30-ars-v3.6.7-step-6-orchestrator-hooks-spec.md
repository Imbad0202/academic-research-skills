...
| L2-5 | JSONL stream tool-event pairing: every non-`agent_message` `item.completed` event is preceded by an `item.started` with the same `item.id` (FIFO per `item.id`); no orphan tool completions; `agent_message` `item.completed` events are exempt (codex emits assistant messages as single completion events) | Invoke `scripts/check_audit_artifact_consistency.py --check A7 --jsonl <path>`; the script implements §3.7 family A row A7 stream-shape rule | §3.7 A7 |
