...
# `set +e` is required: with the §4.1 `set -euo pipefail` shebang, a non-zero
# codex exit would abort the wrapper before `exit_code=$?` runs and prevent
# the §4.6 AUDIT_FAILED artifact set from being written. We capture the code
# locally, then re-enable `set -e` for the rest of the script.
...
- This spec's §1.2 ("two-tier (18 micro + 1 integration)") and §2.1 Q4 row ("18 per-pattern micro-fixtures + 1 chapter-level integration fixture") (two locations) — drafted before §7.1's inventory walk surfaced the off-by-one.
...
**Deliverables:** `scripts/run_codex_audit.sh` AND `scripts/parse_audit_verdict.py` (the JSONL→verdict parser the wrapper invokes per §4.4 Step 4 to convert codex's JSONL `final_message` into the `<run_id>.verdict.yaml` file). Both implement §4 contract end-to-end.
