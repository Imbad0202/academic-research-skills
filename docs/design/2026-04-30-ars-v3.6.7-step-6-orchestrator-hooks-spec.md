...
A canonical no-tool run emits five events in order (the four-event "minimum" pre-0.125 drafts described, plus the `agent_message` `item.completed` they implicitly assumed):
