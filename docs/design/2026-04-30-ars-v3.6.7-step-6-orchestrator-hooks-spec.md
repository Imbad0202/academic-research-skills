...
B1a. **Idempotent run_id de-duplication**: for every candidate proposal in
    ...
    - **Tuple matches** (the persisted entry IS the audit answer for
      ...
      (best-effort, log on failure but do not block), then **GO TO B10
      reading the pre-existing persisted entry's verdict**. This mirrors
      B8b's continuation and closes the false-BLOCK race where a prior
      orchestrator session appended to passport but crashed before B9's
      proposal-move ran: without this branch, B1a would remove the
      proposal from the candidate set, B3 would see "no proposal" and
      BLOCK with `[AUDIT GATE BLOCKED: no audit artifact for ...]`,
      even though the legitimate audit answer is already in the passport.
...
| **P-PB-dup-early** | B1a early idempotent recheck finds passport already has a candidate proposal's `run_id` AND the persisted entry's `(stage, agent, deliverable_sha)` matches the current gate tuple (typical recovery from a prior session's post-B9 / pre-consume crash) | none in this session (the duplicate was committed by the other session) | proposal moved to `consumed/` (best-effort); skip candidate selection; GO TO B10 reading the pre-existing persisted entry's verdict | (silent — proceed using the already-persisted entry's verdict) | continues at B10 |
