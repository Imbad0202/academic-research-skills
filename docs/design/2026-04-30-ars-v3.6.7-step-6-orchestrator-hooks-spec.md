...
# bare semver string, so we strip the "codex-cli " prefix. Future codex
# versions may change the prefix; the awk last-field extractor is robust to
# any "<name> <semver>" format.
_codex_version() {
...
# `--output`, `--stdout-log`, or `--stderr-log` flags in 0.125 — JSONL goes
# to stdout (split via tee into <run_id>.jsonl + <run_id>.stdout) and stderr
# is redirected to <run_id>.stderr at shell level.
#
# `set +e` is required because §4.1 `set -euo pipefail` would abort the
# wrapper on non-zero codex exit before `exit_code=$?` could capture; we
# capture locally then re-enable.
#
# This is `codex exec`, not `codex exec resume` — every audit run is a
# fresh session (no prior SESSION_ID to resume).
...
- §4.4 helper functions: `_atomic_write`, `_sha256`, `_extract_jsonl_session_id`, `_codex_version` (semver extractor — strips the `codex-cli ` prefix from `codex --version` output so the sidecar's `codex_cli_version` field stays bare-semver per §3.4), `_compute_bundle_manifest_sha` per §3.6 contract, `_now_rfc3339_ms`, `_random_4hex` from `/dev/urandom`.
