...
| B7 | `sidecar.run_id` is identical to all four artifact file basenames (`<run_id>.jsonl`, `<run_id>.meta.json`, `<run_id>.verdict.yaml`, `<run_id>.audit_artifact_entry.json`) — one canonical filename convention; bare `<run_id>` only, no stage / agent / deliverable prefix | sidecar (canonical source of `run_id`), all four artifact files | both | wrapper at write time uses `$run_id` for all paths (§4.4); §5.2 L3-1 precondition validates sidecar's own basename against its `run_id` field; §5.2 **L3-8** gating check cross-validates the sidecar's `run_id` against the basenames of all three other artifact files (jsonl / verdict / proposal entry) — closes the "swap one artifact file for a different run's file" forgery seam (F-050 closure: pre-Round-10 the all-four claim was made but only the sidecar half was actually checked) | wrapper aborts with "internal: run_id mismatch"; orchestrator rejects at L3-1 (sidecar self-mismatch) or L3-8 (cross-file basename drift) with "audit fake-detected: artifact filename mismatch" |
...
| 3 | Orchestrator | At stage transition, attempts §5.6 Path A first (look up matching persisted entry by `(stage, agent, deliverable_sha)`). Path A succeeds = entry valid + 10 gates pass + verdict schema valid + mirror match. Failure at any sub-step falls through to Path B |
...
| L3-8 | `sidecar.run_id` equals the bare `<run_id>` basename of the sidecar file AND the bare `<run_id>` basename of every path in `artifact_paths.{jsonl, sidecar, verdict}` AND (for proposal entries observed at Path B) the basename of the proposal entry file itself. All four artifact files share one bare `<run_id>` stem per §3.7 family F (F2/F3) | String parse of basename + equality across all four | rule 7 (new in §3.4 — F-050 closure: B7 enforcement was previously sidecar-only) |
...
- Path A is attempted first whenever a matching persisted entry exists. Path A
  succeeds only when the entry validates AND the ten gating checks pass AND
