...
| **L8** | `handoff_schemas.md` Schema 9 caveat state matches manifest completeness | The manifest's `supported_consumers[]` MUST exactly match one of two closed sets, or L8 fails: <br>(i) **PR-A pre-release set:** `{bibliography_agent}` (single entry). Lint REQUIRES grep-positive for `Consumer-side integration deferred to v3.6.5+` (caveat MUST remain). Caveat retirement is forbidden in PR-A because the second consumer has not shipped. <br>(ii) **PR-B release set:** `{bibliography_agent, literature_strategist_agent}` (both entries). Lint REQUIRES grep-negative for `Consumer-side integration deferred to v3.6.5+` AND grep-positive for backpointer to `literature_corpus_consumers.md`. <br>Any other manifest state (empty, typoed agent name, strategist-only, additional unknown consumer) fails L8 with `manifest does not match a known release state`. |
