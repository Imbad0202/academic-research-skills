...
| 4 | All v3.2 categories present AND every category is UNCERTAIN AND none resolved (no USED, no NOT USED, no `ai_used` input) | Honest "AI-disclosure status not supplied — categories pending confirmation" annotation; renderer prompts the user to run the v3.2 UNCERTAIN-confirmation flow before re-invoking | Pure-UNCERTAIN state has no resolved signal at all; falling through to row 5 would emit "not supplied for this run" without surfacing that the user should re-confirm. |
| 5 | None of rows 1–4 match (i.e., no `ai_used` input AND no USED category AND no UNCERTAIN) | Honest "AI-disclosure status not supplied for this run" annotation | G3 D3 cold-start case. Includes: log absent; log present with all NOT USED (`ai_used` not supplied); cold-start with no input at all. |

Rows 4 and 5 both surface "not supplied" annotations but distinguish *which* state is not supplied (pending confirmation vs no signal at all); row 2 distinguishes a contradiction from either missing-input case. Row 3 admits a render that may still carry per-facet UNCERTAIN annotations — the table is "first match at the whole-disclosure level"; the within-disclosure facet-level annotation policy lives at §4.4 #6 and is bounded by the auto-promotion forbiddance above. Forcing every author to set `ai_used: false` would (a) over-read the anchors (none require positive no-use statements), (b) impose disclosure labour on the majority of corpus entries which involve no AI use, and (c) require adding the very entry-level field this Decision Doc just opted not to add. The opt-in path keeps the explicit-no-use statement available to authors who want it.
