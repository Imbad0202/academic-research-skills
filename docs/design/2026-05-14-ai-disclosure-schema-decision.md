...
1. **`shared/contracts/passport/literature_corpus_entry.schema.json` — NO CHANGE.** No `ai_disclosure` field is added. The current schema (`additionalProperties: false`) does not carry one today and will not gain one at this issue. The renderer (§4 item 6) does not depend on entry-level AI-usage data; its inputs are:
   ...
   - **(c) Paper-level AI-usage categories**: the v3.2 `disclosure` mode Phase 2 categorization (research / drafting / revision / editing / analysis / peer-review-simulation, per `academic-paper/references/disclosure_mode_protocol.md`). USED / NOT USED / UNCERTAIN per category.
   ...
   - **(e) `prompt_record` per LLM/GenAI tool used** (new renderer input, conditional on G5 gate firing): either the full prompt text or the detailed structure description per PRISMA M6.a verbatim. Renderer emits this only when `selected_track == PRISMA-trAIce` AND `tool_type ∈ {LLM, GenAI}` AND the field is supplied; if the gate fires but the field is missing, renderer emits the G3 "not supplied" annotation specifically for the prompt facet (rather than for the whole disclosure).
   - **(f) `level_of_involvement` per category USED** (new renderer input, required when `--policy-anchor=IEEE` is selected): open string aligned with IEEE #5 mandate, with recommended exemplars per G8 (`"full drafting" | "outline only" | "paragraph-level suggestion" | "factual check" | "stylistic revision"`). Renderer emits this verbatim for IEEE; PRISMA-trAIce / ICMJE / Nature renderers may pass it through optionally per G8.
   - **(g) Three-state input completeness flag** (computed by renderer): drives G3 ↔ G10 composition per §2.3 table — "no input + no log" → "not supplied" annotation; "ai_used=false explicit" → "no AI" statement; "ai_used=true or log categories present" → full render.

   Collection path for (d) (e) (f): when the pipeline log already records the data (well-instrumented runs), inputs auto-populate; when invoked cold-start or with incomplete log, the renderer **prompts the user for the missing input** (existing v3.2 Phase 2 UNCERTAIN-confirmation pattern extends to cover the new inputs).
...
6. **Renderer surface — NEW.** Per G1 + G4, the new investment goes here:
   ...
   - **New:** renderer tests covering:
     ...
     - **G5 prompt-disclosure test set**: PRISMA-trAIce track + `tool_type ∈ {LLM, GenAI}` + `prompt_record` supplied → prompts rendered; PRISMA-trAIce track + `tool_type ∈ {LLM, GenAI}` + `prompt_record` missing → "not supplied" annotation for the prompt facet only (rest of disclosure still renders); general track + any `tool_type` → prompts omitted from render.
     - **G8 level_of_involvement test set**: `--policy-anchor=IEEE` + `level_of_involvement` supplied → emitted verbatim; `--policy-anchor=IEEE` + missing → "not supplied" annotation for the level facet only; other anchors + supplied → optional pass-through.
     - **G3 ↔ G10 three-state test set** (mirrors §2.3 table): no input → "not supplied" annotation; explicit `ai_used: false` → "no AI was used" statement; `ai_used: true` or categories detected → full render.
