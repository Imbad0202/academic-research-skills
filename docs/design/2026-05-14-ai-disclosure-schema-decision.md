...
|---|---|---|---|
| 1 | `ai_used: false` supplied explicitly AND every supplied v3.2 category is NOT USED (no contradiction) | "No AI was used in preparing this paper" statement (G10 opt-in path) | Explicit no-AI is a positive author statement; row consistency check guards against `ai_used:false` paired with a contradicting USED category (which is a data-integrity bug, surfaced as row 2 below). |
| 2 | `ai_used: false` supplied AND ≥1 v3.2 category is USED (contradiction) | Honest "AI-disclosure conflict — explicit no-AI input contradicts USED category" annotation; renderer prompts the user to reconcile | Refusing to silently pick one resolution side preserves authorship accountability (mirrors the G4 venue+anchor-conflict invariant in §4.4 #7). |
| 3 | `ai_used: true` supplied OR ≥1 v3.2 category is USED, AND no row-2 contradiction | **Full anchor-specific disclosure render**, applying the §4.4 #6 per-facet UNCERTAIN handling rule chosen by the implementation for any *still-unresolved* UNCERTAIN categories in this run. The G3/G10 invariant (UNCERTAIN MUST NOT auto-promote to USED) constrains §4.4 #6 — acceptable per-facet rules include: prompting user to resolve all UNCERTAIN before render, surfacing per-facet "not supplied" annotations alongside USED facets, or marking the whole disclosure draft until all UNCERTAIN resolve. **Forbidden:** rendering UNCERTAIN facets as though USED. | G4 E2 + G5–G9 normal path. The Decision Doc does not pick among the per-facet alternatives; it only forbids the auto-promotion case. |
| 4 | ≥1 v3.2 category is UNCERTAIN AND no v3.2 category is USED AND no `ai_used` input (covers both "all UNCERTAIN" and "mix of NOT USED + UNCERTAIN with no USED") | Honest "AI-disclosure status not supplied — categories pending confirmation" annotation; renderer prompts the user to run the v3.2 UNCERTAIN-confirmation flow before re-invoking | At least one category needs resolution. Distinguished from row 5 (no UNCERTAIN at all) because here the user *should* re-confirm; row 5 has no signal to confirm. |
| 5 | Every supplied v3.2 category is NOT USED AND no UNCERTAIN AND no `ai_used` input (de facto no-AI run with no explicit author opt-in) | **Silence — emit no AI-disclosure statement at all** (G10 "silence is default-OK") | G10 Decision: anchors describe forward-looking obligations conditional on AI use. When categorization is complete and uniformly NOT USED, there is nothing to disclose; emitting "not supplied" would mis-frame a positive no-use signal as a missing signal. The explicit `ai_used: false` opt-in (row 1) is available to authors who want a positive statement. |
| 6 | None of rows 1–5 match (no `ai_used` input, no USED, no UNCERTAIN, no NOT USED — i.e., no v3.2 categorization run at all) | Honest "AI-disclosure status not supplied for this run" annotation | G3 D3 pure cold-start case: log absent; cold-start invocation with no v3.2 Phase 2 categorization performed; no input dimension carries any signal. |

Rows 4, 5, and 6 distinguish three "no AI declared at the disclosure-statement level" states: row 4 has unresolved UNCERTAIN (user should re-confirm), row 5 has a complete categorization that is uniformly NOT USED (silence per G10), row 6 has no categorization signal at all (pure cold-start). Row 2 distinguishes a USED-vs-`ai_used:false` contradiction from either missing-input case. Row 3 admits a render that may still carry per-facet UNCERTAIN annotations — the table is "first match at the whole-disclosure level"; the within-disclosure facet-level annotation policy lives at §4.4 #6 and is bounded by the auto-promotion forbiddance above. Forcing every author to set `ai_used: false` would (a) over-read the anchors (none require positive no-use statements), (b) impose disclosure labour on the majority of corpus entries which involve no AI use, and (c) require adding the very entry-level field this Decision Doc just opted not to add. The row-1 opt-in path keeps the explicit-no-use statement available to authors who want it; the row-5 silence path implements G10's "default-OK" stance for unambiguous no-use runs.
...
- **G3 / G10 invariant.** The five-row precedence table in §2.3 (whole-disclosure output decision) MUST hold. The auto-promotion forbiddance: an **individual** v3.2 category that is still UNCERTAIN MUST NOT be rendered as though USED in any of the four anchor outputs. Partial-state cases (≥1 USED with one or more still-UNCERTAIN) are admitted into §2.3 row 3 "full anchor-specific disclosure render"; the per-facet handling of the still-UNCERTAIN categories inside that render is the implementation issue's choice per §4.4 concern #6, bounded by this auto-promotion forbiddance.
