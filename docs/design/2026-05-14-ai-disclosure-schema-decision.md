...
**Three-state renderer-input contract (resolves G3 ↔ G10 composition).** Rows are evaluated in priority order from top to bottom; the renderer emits the output of the **first** row whose precondition matches. Subsequent rows do not fire. This precedence makes the rows mutually exclusive in practice; the rationale for each precedence level is in the third column.

| # | Precondition (first match wins) | Renderer output | Precedence rationale |
|---|---|---|---|
| 1 | Any v3.2 category is UNCERTAIN AND unresolved | Honest "AI-disclosure status not supplied — categories pending confirmation" annotation; renderer prompts the user to run the v3.2 UNCERTAIN-confirmation flow before re-invoking | UNCERTAIN must not auto-promote to either USED or NOT USED (G3/G10 invariant). Resolving the UNCERTAIN flow re-enters the table at row 2/3/4/5. |
| 2 | `ai_used: false` supplied explicitly AND every supplied v3.2 category is NOT USED (no contradiction) | "No AI was used in preparing this paper" statement (G10 opt-in path) | Explicit no-AI is a positive author statement; row consistency check guards against `ai_used:false` paired with a contradicting USED category (which is a data-integrity bug, surfaced as row 3 below). |
| 3 | `ai_used: false` supplied AND ≥1 v3.2 category is USED (contradiction) | Honest "AI-disclosure conflict — explicit no-AI input contradicts USED category" annotation; renderer prompts the user to reconcile | Refusing to silently pick one resolution side preserves authorship accountability (mirrors the G4 venue+anchor-conflict invariant in §4.4 #7). |
| 4 | `ai_used: true` supplied OR ≥1 v3.2 category is USED (with no UNCERTAIN remaining and no row-3 contradiction) | Full anchor-specific disclosure render | G4 E2 + G5–G9 normal path. |
