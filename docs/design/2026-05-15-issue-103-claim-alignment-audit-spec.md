...
- UAF-INV-5: `rationale` MUST begin with the row's own `fault_class` value followed by `": "` and a free-form detail. Mirrors INV-14 rationale prefix requirement on the cited path; the prefix must match this row's `fault_class` field, not just any known tag.
