...
- **Known carve-out — `draft_writer_agent` (Bucket A) carries a two-phase static union (#330; logged 2026-06-16).** Of the 23 Bucket A agents, `draft_writer_agent` is the one exception to "single own-phase glob": its manifest `allowed_write_globs` is `["phase4_*/**", "phase6_*/**"]`, because it genuinely drafts at Phase 4 and revises at Phase 6 per invocation (the #390 patch-document path writes `phase6_*/revision_patch_round<N>.json` via the Write tool). Because the PreToolUse payload carries only `agent_type` and not the invocation phase, the guard cannot tell a Phase 4 draft call from a Phase 6 revision call — so this static union permits cross-writes within the phase4↔phase6 pair, the same fence-weakening the bullet above uses to justify exempting the Bucket B agents (`report_compiler` shares the identical Phase 4,6 signature). This is a **documented, bounded** Slice-1 limitation. Owner decision (#330): **keep-and-document** rather than (a) narrow to `phase4_*/**` now — which would deny the real #390 phase6 revision write — or (b) reclassify to Bucket B — which would drop the fence (and the wholesale-Bash-deny) entirely. The real fix is the Slice 3 per-invocation grant (§5 item 3): once it lands, `draft_writer_agent` narrows to `phase4_*/**` and routes its Phase 6 writes through the authenticated grant. The carve-out is canonically recorded in `scripts/ars_phase_scope_manifest.json` (`_known_multiphase_static_union`).
...
3. **Slice 3 — Bucket B per-invocation scope grants.** Only for `report_compiler`, `devils_advocate`, `argument_builder`, `visualization` (and the `draft_writer_agent` Bucket A carve-out per #330 — once grants exist it narrows from its `phase4_*/**`+`phase6_*/**` static union to `phase4_*/**` and routes Phase 6 writes through the grant). Requires a current-task-scope record the hook reads — this is the *minimal* form of a "task envelope": just enough machine-readable scope for the hook, not a full contract. **NOT independently shippable on its own:** it has a hidden trust dependency — *who writes the grant, when it expires, and how an agent is prevented from editing its own grant*. The grant producer must be the dispatching layer (not the dispatched agent), and the grant store must be under Step 2 self-protection. This slice is effectively a minimal conductor/envelope and should only be built once that authenticated-grant-producer + protected-grant-store trust boundary is designed (its own design round).
