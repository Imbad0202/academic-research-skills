...
| `draft_writer_agent` | A | Phase 4 (drafting) **or** Phase 6 (revision), per invocation | hard-fence (audit existing v3.6.6/v3.6.7 partial language) | bounded phase4↔6 static-union inflation (#330): dual-phase per invocation but a single `agent_type` can't gate phase, so its manifest glob is `phase4_*/**`+`phase6_*/**`; kept-and-documented, real fix = Slice 3 per-invocation grant | academic-paper/SKILL.md row 5 |
