...
| **3'. RE-REVIEW** | `academic-paper-reviewer` v1.10.0 (re-review) | VERIFIED_ONLY | **Verification package** (per re-review mode spec in `academic-paper-reviewer/SKILL.md`): Revision response checklist + residual issues list + new Decision (Accept / Minor / Major) + **R&R Traceability Matrix (Schema 11)** with Author's Claim + Verified? columns | **Narrow re-review team**: eic_agent + editorial_synthesizer_agent (2 agents — not the full Stage 3 panel; the Round-1 Reviewer Configuration Cards are reused and field_analyst_agent is NOT re-run, per `re_review_mode_protocol.md` § Yardstick Continuity; a user-requested fresh full review at 3' — mid-entry quick→full path — runs full mode with field_analyst instead); **👁 collaboration_depth_agent (v3.5.0, advisory)** | 🧑 **Decision-heavy checkpoint:** user reviews verification decision. Hard cap: **max 1 RE-REVISE round; 2 revision loops total** across Stages 4 + 4'. Major outcome at 3' → Residual Coaching → Stage 4'. 👁 Observer runs post-checkpoint; never blocks |
