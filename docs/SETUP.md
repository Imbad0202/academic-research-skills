...
| `ARS_MODEL_TIERING` | Unreleased (#517) | Opt-in model tiering: `economy` (frontier session — execution-type agents step down one tier, floor Opus-class) or `quality-boost` (below-frontier session — judgment-type agents step up to the frontier tier at the gates). Unset = session model everywhere; unknown values warn and behave as unset. | `shared/model_tiering.md` |
