...
| `ARS_CACHE_REVALIDATE=1` | Unreleased (#541) | Opt-in live re-verification: a reference that supports a HIGH-IMPACT claim (#549 tier) AND is past the staleness threshold is verified live (cache bypassed, then re-populated) instead of cache-served. Default off = advisory-only. | `academic-pipeline/agents/integrity_verification_agent.md` § A0.5 |
