...
6. **Record per run** (committed): write `runs/<date>-<fixture>-<baseline|post>-r<k>.json`
   ...
   **Blocked-run separation:** if a fail-loud checker stops the panel before all
   five cards and synthesis exist, do not write a normal scored-run record. Preserve
   the partial evidence under `runs/raw/blocked/<same-stem>/` and a status record
   under `runs/blocked/<same-stem>.json`. Blocked attempts are operational evidence,
   not zero-valued measurements: exclude them from all means, do not impute missing
   seats, and do not launch a replacement draw to conceal the abort.
...
| 2026-07-27 | 19bc872 (Spec A implementation, including terminal DA-contract correction) | claude-opus-5 (effort xhigh, thinking enabled; same isolated per-seat two-phase dispatch) | **BLOCKED:** 2 clean panels launched; r1 completed, r2 conformance-aborted; MS01/MS02 not launched | **NOT COMPUTABLE** | **NOT COMPUTABLE** | r1: **1** (descriptive only; panel decision `major_revision`); replicate mean **NOT COMPUTABLE** | **NOT COMPUTABLE** | Formal Spec-A E4 attempt stopped without replacements after r2's Perspective Phase 2 emitted an empty `## Scoring Plan Dissent` section and failed `[DISSENT-GRAMMAR: dissent section must name dimension_id]`. Phase 2 retry is permitted only for multi-dissent, so DA and synthesis were not run. Observed clean-cohort panel-abort rate **1/2 = 0.50** versus the Spec-A diagnostic expectation of approximately zero. Required 2 × 3 fleet and all acceptance gates are therefore **BLOCKED / NOT COMPUTABLE**, not pass or fail. Completed r1 record + raw panel and the r2 status record + partial raw bundle are committed under `runs/`; no replacement draw or missing-value imputation was used. |
