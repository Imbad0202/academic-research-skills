...
The scenario content must be synthetic or explicitly authorized. Dispatch is
manual and requires operator consent for provider, content class, and estimated
cost. Raw subject outputs, judge outputs, the exact execution manifest, and the
final `heldout-measurement/1.1` report are retained. Until such a valid report is
committed, ARS may say the mechanism is implemented but must describe its
behavioral effect as **unmeasured**.
