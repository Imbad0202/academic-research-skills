...
- **Cross-model Gemini guard requires a valid supported chunk index; OpenAI text extractor tolerates a non-string `text` (#351, post-ship review of #349).** The post-squash review of #349 surfaced two residual malformed-response paths the four review rounds missed. (1) `gemini_is_grounded.jq` only checked that `groundingSupports` was a non-empty array — a `groundingSupports: [{}]`, or one whose only `groundingChunkIndices` are empty / negative / string / out-of-range, still passed the guard even though it links the verdict to no retrieved chunk. The source filter blanks those indices, but the blank-source downgrade only rescues `VERIFIED`, so an ungrounded `NOT_FOUND` / `MISMATCH` could still be trusted as a grounded cross-model result. The guard now requires at least one **valid** supported chunk index, using the same in-range non-negative-integer predicate as `gemini_sources.jq` — so guard-pass ⟺ at least one source is extractable (a cleaner contract than before). (2) `openai_text.jq` crashed `join` (`"" + {}`) on a non-string `output_text.text`; it now filters `.text` to strings, mirroring the URL string-filter. +7 behavior tests (no-valid-index ×5 parametrized, the guard⟺sources invariant, the text-crash path). Both holes and both fixes verified first-party.
