...
- **#184 Phase 1a — citation-extraction gold subset.** New top-level `evals/` directory holds v3.10 generalized gold-set corpora for `verification_gate.verify_citation` measurement targets. Ships `evals/gold/citation_extraction/` with 50 hand-curated tuples + `manifest.yaml` + `expected_outcomes.json` (Phase 1b will populate tuples — this PR ships the structural validator + fixture). v3.10.0 binding thresholds: aggregate `accuracy >= 0.90` across 50 tuples, per-class `accuracy >= 0.85` for each of `true` / `false` / `unresolvable` (changing requires spec amendment per #184 §3.1.1 / E-V2). Distribution: 20 valid_doi + 10 valid_arxiv + 10 valid_unresolvable + 5 manual_exempt + 5 fabricated. Tuple shape (locked per codex consult Q1-Q5): self-contained `corpus_entry` mirroring `literature_corpus_entry`, `arxiv_id` as tuple-level field (forward-looking — see #234 for #182 implementation alignment), `human_expert_verdict` optional (10/50 = 20% per Delta 5), `fabrication_intent` boolean enforced on fabricated tuples, `provenance_note` required on valid_unresolvable tuples with date regex check. New `scripts/check_evals_gold_set.py` enforces 9 invariants (I1 set equality / I2 tuple_id ↔ filename / I3 kind distribution / I4 no-dup-JSON-keys / I5 label ↔ kind / I6 arxiv_id placement / I7 fabrication_intent marker / I8 provenance_note regex / I9 resolver_outcomes shape) via 16 mutation tests on a 4-tuple clean fixture. CI step wired into `.github/workflows/spec-consistency.yml`. Phase 1b (harness `scripts/run_evals.py` + output schema `shared/evals_lift_report.schema.json` + standalone CI workflow `.github/workflows/eval-harness.yml` + lift gate `scripts/check_ranking_lift.py` + remaining 40 `human_expert_verdict` entries) lands in a follow-up PR. Spec: `docs/design/2026-05-21-v3.10-184-extend-eval-harness-spec.md`.
