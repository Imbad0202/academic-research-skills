...
- **Layer-5 contribution-significance probes extended into plan mode and revision coaching (#393, blindspot-audit F-2, adjudicated shape 1).** ARS quality machinery was defect-oriented end-to-end — a paper could pass every gate and still be a micro-extension, because the only constructive contribution coaching (socratic_mentor Layer 5, SIGNIFICANCE & CONTRIBUTION) lived at the RQ-incubation stage. Two later-stage surfaces now carry the same question patterns: **(1)** `academic-paper` plan mode gains Step 2.5 CONTRIBUTION SHARPENING between chapter negotiation and the argument stress test — the mentor probes the contribution argument the fresh outline implies ("Ten years from now, what will citers say this paper established?", "Remove this paper from the literature — what is missing?"), anchored to the user's own Chapter Summaries, extracting `[INSIGHT: contribution_claim]`; **(2)** `academic-paper-reviewer` Phase 2.5 gains step 3, a contribution framing probe alongside the existing prioritization steps (5→6 steps; no external step-number references existed). Single-sourcing: Layer 5 owns the pattern family — both surfaces are labeled instantiations with an explicit re-derive-don't-fork note, and Layer 5 itself gains a downstream-consumers note so future pattern edits propagate instead of forking. The boundary is load-bearing (Kong L2 verb test, per the #393 adjudication that rejected shapes 2–3): questions only — never propose, substitute, rank, expand, or select a contribution claim; if the user cannot answer, the open question is recorded and carried into the next step, not filled in by the mentor. Prose-layer only; no schema, scoring, or agent-roster change. The orchestrator's Phase 2.5 coaching sketch (`pipeline_orchestrator_agent.md`) is an abbreviated illustration deferring to the reviewer SKILL.md step list and was deliberately left untouched (line-budget surface, outside the adjudicated two-surface scope).
