...
- **Defrift lock for the #514 tools allowlist (#524).** New `scripts/check_tools_allowlist.py` + a 31-test suite (a failing witness per invariant branch), wired into spec-consistency.yml and the unified pytest manifest. Invariant 1 pins the `tools: Read, Write, Edit, Grep, Glob` frontmatter line on all six #514 surfaces at two layers: an exact raw-line pin (CR-sensitive, so a symmetric LF→CRLF conversion is drift, with a broadened key count so a quoted `"tools":` duplicate cannot hide behind the pinned line) plus a semantic YAML belt (the parsed `tools` value must normalize to exactly the canonical five — duplicate keys resolve last-wins, so a smuggled second key diverges semantically even when a line-level trick slips past). This closes the drift scenario where a future PR edits a source+mirror pair symmetrically (re-adding Bash, dropping a tool, or typoing a name) and passes every CI gate green, because `check_agents_mirror_sync.py` pins only pairwise byte-equality and the runtime guard keys on agent name, never frontmatter; changing the allowlist now requires touching the lint's pinned value in the same commit (standard lock semantics). Invariant 2 reconciles the frontmatter channel against the runtime channel: any agent whose `name` is a Bucket A key in `scripts/ars_phase_scope_manifest.json` must not declare Bash in a `tools:` key in any YAML-legal form — comma string, quoted string, flow/block list, inline comment, `Bash(...)` permission specifier (`BashOutput` is a different tool and not flagged) — failing closed on a missing/non-mapping manifest, unparseable Bucket A frontmatter, or an unrecognized `tools` shape. The YAML-form bypass class and the CRLF/duplicate-key line-scan gaps were found by the codex `gpt-5.6-sol` xhigh review of the first cut (convergent with the first-party security pass) and closed before merge. Plus the one-line allowlist mention the #521 review flagged as absent: `docs/PERFORMANCE.md` / `docs/PERFORMANCE.zh-TW.md` § plugin agents and the SessionStart announce script's plugin-agents line.
