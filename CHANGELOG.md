...
- **Re-review judge independence + judge record (#539).** The Stage 3' re-review both drives the final decision and reports it — exactly the §8.1.2 over-optimization configuration. With `ARS_CROSS_MODEL`, every Priority 1 roadmap item's addressed-ness verdict gets an independent cross-model pass (dispatching-layer transport per #523; divergences marked `[CROSS-FAMILY-DIVERGENCE]` on the R&R Traceability Matrix — a review trigger for the EIC, never a vote). Single-provider runs carry a never-omitted disclosure line instead of silently implying independence. Both cases emit a Judge Record (verification judge vs revision-driving judge, rubric version, evidence seen, judging budget separate from generation) — Schema 6 optional `judge_record`. The reviewer's shipped calibration mode is cited as the same section's calibration-against-a-verifiable-subset safeguard. Authority: `re_review_mode_protocol.md` § Judge Independence. External motivation: Ren et al. (2026, arXiv:2607.13104 §8.1.2).
