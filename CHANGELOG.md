...
- **Opt-in Codex subscription transport for the cross-model verifier (#518, PR #567).** The cross-model citation verifier — ARS's only billable LLM surface — can now run web-grounded verification through a Codex subscription instead of an `OPENAI_API_KEY`. New adapter `scripts/cross_model_codex_verify.sh` (isolated `codex exec --json` in a non-git temp dir with user config and rules ignored; fail-closed grounding guard — `searched=true` only on an observed web-search event, and a `VERIFIED` carrying zero http(s) source URLs downgraded to `NOT_SEARCHED`; zero new inline jq, so the sync lint's guarded-token wiring is untouched) plus smoke-test mirror `scripts/cross_model_smoke_test_codex.sh`. Agent auto-wiring sits behind a new opt-in `ARS_CROSS_MODEL_TRANSPORT=codex` arm in the § Detecting Available Models block (requires the codex CLI + `~/.codex/auth.json`; fail-closed to the existing single-model warning; unset = byte-equivalent pre-change behavior), with quick-setup subsections in `docs/SETUP.md`/`docs/SETUP.zh-TW.md` (byte-identical bash blocks, parity-lint green) and an end-to-end local guide at `docs/LOCAL_USAGE.md` documenting the transport's verified limits (no model, effort, or service-tier echo in `codex exec --json` output).
...
- **`gpt-5.6-sol` promoted `provisional` → `validated` by the first § Promotion Bakeoff run (#518, PR #567).** 30-reference sha256-pinned probe set (`fixture_version 2.0.0`), 180 same-day paired calls (2 models × 3 repeats) through the shipped Codex-transport adapter; all five non-inferiority thresholds pass with no measure near tolerance. Harness and raw run record committed under `evals/bakeoff/cross_model_promotion/` for deterministic offline re-scoring; report at `audits/cross-model-promotion-bakeoff-gpt-5.6-sol-2026-07-16.md`. The recommended default deliberately stays `gpt-5.5`: a bare non-inferiority pass never flips the default per the spec's two-promotion split, and the one observed divergence — the baseline returning `NOT_FOUND` 3/3 with zero source URLs on a verified-real JACS reference the candidate verified 3/3, kept in the fixture as a permanent canary probe — is a single reference (Fisher exact p = 1.0). The § Promotion Bakeoff entry gate now names both transport smoke tests with the Codex route's model-id/effort echo checks marked transport-limited (audit Finding 7); the audit also records a spec defect — measure 2's ±5 pp tolerance cannot bind on a 10-item denominator — left to the spec's reserved threshold-tuning path.
