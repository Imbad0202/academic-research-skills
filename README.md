...
> **Which enforcement mechanisms operate in *your* install channel?** The write-scope guard, slash commands, tools allowlist, subagent orchestration, and script-backed checks vary by channel (plugin / skills copy / repo clone / Cowork / claude.ai Project / Claude Science / Pi). See the per-channel map: [docs/CONTROL_AVAILABILITY.md](docs/CONTROL_AVAILABILITY.md).
...
**👉 [docs/SETUP.md](docs/SETUP.md)** — full guide: install Claude Code, set up API keys, optional Pandoc/tectonic for DOCX/PDF, cross-model verification (`ARS_CROSS_MODEL`), and six installation methods (Plugin, project skills, global skills, claude.ai Project, repo-cloned, Claude Science import). Per-channel enforcement availability: [docs/CONTROL_AVAILABILITY.md](docs/CONTROL_AVAILABILITY.md).
