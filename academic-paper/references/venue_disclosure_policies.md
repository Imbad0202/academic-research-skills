...
| Required phrasing elements | For manuscript-preparation AI: platform/program/tool name, model or tool version **and extension number(s) when applicable**, manufacturer, date(s) of use, a description of how AI was used and on which portions of the manuscript/content, confirmation that the authors reviewed and confirmed the accuracy of generated content, and confirmation that they take responsibility for its integrity. For any scientific-study AI use, describe the specific research use and evaluate the two rights predicates below. If copyright-protected content was entered into the model/tool, **include a copy** of the copyright-holder permission/license with the submission **and separately describe** that permission/license in Methods. If AI-generated text, images, or multimedia are included in the submitted work, state the rights or permission to publish **as determined by the AI service or owner** in Methods or the relevant legend. Only for a study using an LLM: also report platform/program/tool name, version, manufacturer, date(s), prompt(s), their sequence, and any prompt revisions made in response to initial outputs. |
...
| Notes | ICMJE member journal — use the ICMJE umbrella recommendations (see ICMJE row) alongside JAMA's Instructions for Authors; the database does not assign silent precedence between them. This row feeds an **AI-use disclosure renderer**, not a complete JAMA Methods/reporting checklist: it covers the two AI-specific conditional rights/licensing facts above, then adds a scope note directing the author to the remaining current study-design, reporting-guideline, ethics, data-sharing, and other applicable rights/licensing instructions. The separate statement that AI/LLMs/chatbots "should not" generate or format references is surfaced as a non-blocking advisory warning; it is not promoted to a prohibited-use halt or a disclosure-rendering prerequisite. |
