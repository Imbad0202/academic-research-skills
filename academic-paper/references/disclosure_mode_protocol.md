...
| Frontiers | A `GENAI_OR_AIGC` use record is `USED` | For visual records, record whether the use affected a figure. If true, record `figure_represents_data`; originality/plagiarism-free confirmation is required for every figure, while accuracy-to-data confirmation is required only when `figure_represents_data` is true. `KNOWN(false)` for an applicable affirmative fact is incompatible and halts. `OTHER_AI` records never enter these GenAI predicates. |
...
9. After expanding all conditional facts, rerun the Phase-2a.1 compatibility sweep against each use record. A record with an unresolved discriminator, an unmodelled use/artifact class, an `OTHER_CONFIRMED` operation/target whose predicate-specific meaning is unresolved, or a fact combination that conflicts with the selected row halts before Phase 3. Do not infer a new hard condition from policy prose; report a contract gap when the executable table and evidence row cannot be reconciled.
