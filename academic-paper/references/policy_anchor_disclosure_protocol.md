...
**Concern #10 resolution (`ai_used: true` substantive-content gate):** when `ai_used: true` is supplied but no v3.2 category is marked USED and no Phase 2 categorization has been performed (bare-flag input only), row 4 does not emit a complete render. Instead, the bare flag is treated as a **prompt-trigger** that halts the current run and asks the user to complete v3.2 Phase 2 categorization. Once categories are supplied, re-evaluate against the §2 table from the top. This implements the concern #10 resolved path: "force v3.2 categorization flow", consistent with concern #6's honest-signal-preservation discipline.
...
When the user passes both `--venue=<v>` and `--policy-anchor=<a>`, the renderer evaluates whether the two map to compatible policies. If `--venue=Nature` and `--policy-anchor=nature` → consistent, proceed (both target Nature's substantive policy via the shared source pointer). If `--venue=Nature` and `--policy-anchor=IEEE` → conflicting (Nature requires Methods placement; IEEE requires acknowledgments-only) → **reject with explicit error** listing the policy conflict.
