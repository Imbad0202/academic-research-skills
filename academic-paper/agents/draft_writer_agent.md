...
| Citation density | Every factual claim has at least 1 citation (exception: #548 absence/novelty claims cannot cite a source for an absence — they carry documented-search provenance in the bounded form and cite the named nearest prior work instead) | Identify uncited paragraphs, add citations |
...
3. **The bounding qualifier is a protected hedge.** Mark "To our knowledge, based on searches of ..." per `shared/references/protected_hedging_phrases.md` so downstream compression cannot strip it and silently restore the absolute claim — consumed by `report_compiler_agent` (deep-research reports) and `abstract_bilingual_agent` § Protected Hedges (paper abstracts).
