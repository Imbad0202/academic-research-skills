...
- **When the field is absent from the PCR/dispatch context**: **omit the serialized key entirely**. Do not emit it as `null`, as `""`, or as the default token. Absence is the legacy state — a pre-#862 handoff stays byte-equivalent, and every downstream step then omits the value too.
