...
- Ask about the output language pair (`output_language_pair`): the two languages of the abstract surfaces. Default `zh-tw-en` (Traditional Chinese L1 + English L2) — the pre-#862 behaviour. Per-run override; the answer must be a token in the registry of `shared/output_language_pair.md` (an unsupported, non-string, or empty value fails visibly naming that registry — never a silent fallback to the default).
...
| **Output Language Pair** | [zh-tw-en (default) / per-run registry token — ROW OMITTED ENTIRELY when the run declares no pair, so a pre-#862 PCR stays byte-identical; absence means the default entry `zh-tw-en` and every consumer then omits the value] |
