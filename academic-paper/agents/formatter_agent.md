...
- LOW-WARN (rule 4): run `/ars-mark-read <slug>` to acknowledge — optionally with a #513 read-scope attestation (`--scope`, `--locator`, `--note`). A partial-coverage attestation (`abstract_only`/`toc_only`, or `sections` whose locators do not cover this citation's anchor) keeps the marker LOW-WARN with an explicit coverage note instead of promoting; read the relevant part of the source (or attest `full_text`) for promotion.
